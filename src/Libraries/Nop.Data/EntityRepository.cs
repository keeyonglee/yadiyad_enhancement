...
namespace Nop.Data
{
    ...
    public partial class EntityRepository<TEntity> : IRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        public virtual IList<TEntity> EntityFromSql(string storeProcedureName, params DataParameter[] dataParameters)
        {
            ...
            {
                if (dataParameter.DataType == DataType.NVarChar)
                {
                    parameterTexts += $" ,{("\"" + dataParameter.Value + "\"" ?? "null")}";
                }
                else
                {
                    parameterTexts += $" ,{(dataParameter.Value ?? "null")}";
            ...
            parameterTexts = parameterTexts.Substring(2, parameterTexts.Length - 2);

            Debug.WriteLine($"CALL {storeProcedureName} ({parameterTexts})");
