...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShipmentService : IShipmentService
    {
        ...
        private DateTime GetDateTimeFromEatsDeliveryDateTimeSlot(string deliveryDateText, string deliveryTimeText)
        {
            string[] time = deliveryTimeText.Split(' ');

            var dateString = $"{deliveryDateText} {time[0]}";
            var checking = DateTime.TryParse(dateString, out var correctDate);
        ...
        public virtual IList<Shipment> GetReturnShipmentsByOrderId(int orderId, bool? shipped = null, int vendorId = 0)
        {
            if (orderId == 0)
                return new List<Shipment>();
            ...
            var shipments = from s in _shipmentRepository.Table
                            ...
                            && s.ShipmentType == ShipmentType.Return
                            select s;
        ...
        public virtual IList<Shipment> GetShipmentsByOrderId(int orderId, bool? shipped = null, int vendorId = 0)
        {
            ...
            var shipments = _shipmentRepository.Table
                .Where(s => s.Type == (int)ShipmentType.Fulfillment);
        ...
        public virtual IList<Shipment> GetShipmentsByReturnOrderId(int returnOrderId, bool? shipped = null, int vendorId = 0)
        {
            if (returnOrderId == 0)
                return new List<Shipment>();
            ...
            return shipments.Where(shipment => shipment.ReturnOrderId == returnOrderId).ToList();
        }
            ...
            bool ignoreShipped, bool ignoreDelivered)
        {
            ...
            query = from si in query
                ...
                && s.Type == (int)ShipmentType.Fulfillment
                    select si;
        ...
        public IList<Shipment> GetPendingReturnShipments(DateTime cutOffDate)
        {
            var shipments = from s in _shipmentRepository.Table
                join o in _orderRepository.Table on s.OrderId equals o.Id
                where s.ShippedDateUtc == null
                      && s.Type == (int) ShipmentType.Return
                      && s.CreatedOnUtc < cutOffDate
                      && o.OrderStatusId == (int)OrderStatus.ReturnRefundProcessing
                select s;
            return shipments.Distinct().ToList();
        }

        public string CreateLalamoveScheduleAt(string deliveryDate, string deliveryTime)
        {
            ...
            var scheduleAtDate = GetDateTimeFromEatsDeliveryDateTimeSlot(deliveryDate, deliveryTime).AddHours(-8);
            var scheduleNow = DateTime.UtcNow;
            if (scheduleAtDate > scheduleNow)
            {
                return scheduleAtDate.ToString("yyyy-MM-ddTHH:mm:ssZ");
            }
            else
            {
                return null;
        ...
        public DateTime? GetDeliverySlot(string deliveryDateText, string deliveryTimeText)
        {
            if (string.IsNullOrEmpty(deliveryDateText))
                throw new ArgumentNullException($"{nameof(deliveryDateText)} cannot be null");

            if (string.IsNullOrEmpty(deliveryTimeText))
                throw new ArgumentNullException($"{nameof(deliveryTimeText)} cannot be null");

            return GetDateTimeFromEatsDeliveryDateTimeSlot(deliveryDateText, deliveryTimeText);
        }

        public Shipment GetShipmentByTrackingNumber(int trackingNumber)
        {
            if (trackingNumber == 0)
                throw new ArgumentException($"{nameof(trackingNumber)} cannot be 0");

            return (from s in _shipmentRepository.Table
                where s.TrackingNumber == trackingNumber.ToString()
