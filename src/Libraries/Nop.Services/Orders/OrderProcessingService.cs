...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected class PlaceOrderContainer
        {
            ...
            public decimal VendorShippingDiscount { get; set; }
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Details</returns>
        protected virtual PlaceOrderContainer PreparePlaceOrderDetailsWithvendor(int vendorId, ProcessPaymentRequest processPaymentRequest, IEnumerable<int> itemIds)
        {
            var details = new PlaceOrderContainer
            {
                //customer
            ...
            //load shopping cart
            details.Cart = _shoppingCartService.GetShoppingCartWithItemIds(details.Customer, itemIds, ShoppingCartType.ShoppingCart, processPaymentRequest.StoreId);
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                ...
                {
                    details.ShippingMethodName = shippingCarrier.Name;
                    details.ShippingRateComputationMethodSystemName = shippingCarrier.Name;
                }
            ...
            //order total (and applied discounts, gift cards, reward points)
            var orderTotal = _orderTotalCalculationService.GetShoppingCartTotalByVendor(vendorId, details.Cart, out var orderDiscountAmount, out var orderAppliedDiscounts, out var shippingDiscount, out var platformShippingDiscount, out var platformSubtotalDiscount, out var appliedGiftCards, out var redeemedRewardPoints, out var redeemedRewardPointsAmount);
            //var orderTotal = _orderTotalCalculationService.GetShoppingCartTotal(details.Cart, out var orderDiscountAmount, out var orderAppliedDiscounts, out var appliedGiftCards, out var redeemedRewardPoints, out var redeemedRewardPointsAmount);
            if (!orderTotal.HasValue)
                throw new NopException("Order total couldn't be calculated");
            ...
            details.AppliedGiftCards = appliedGiftCards;
            details.OrderTotal = orderTotal.Value;
            details.VendorShippingDiscount = shippingDiscount;
            details.PlatformShippingDiscount = platformShippingDiscount;
            details.PlatformSubTotalDiscount = platformSubtotalDiscount;
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            var order = new Order
            {
                ...
                VendorShippingDiscount = details.VendorShippingDiscount,
                PlatformShippingDiscount = details.PlatformShippingDiscount,
            };
        ...
        /// <param name="details">Details</param>
        /// <returns>Order</returns>
        protected virtual Order SaveOrderDetailsWithVendor(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainer details)
        {
            var order = new Order
            {
                StoreId = processPaymentRequest.StoreId,
                ...
                VendorShippingDiscount = details.VendorShippingDiscount,
                PlatformShippingDiscount = details.PlatformShippingDiscount,
                PlatformSubTotalDiscount = details.PlatformSubTotalDiscount
            };
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
            ...
            {
                order.CancellationDateUtc = DateTime.UtcNow;
            }
        ...
        public virtual void CheckOrderStatus(Order order)
        {
            ...
            if (order.PaymentStatus == PaymentStatus.Paid && !order.PaidDateUtc.HasValue)
            {
                ...
                //assumption: one vendor one order.
                var vendors = GetVendorsInOrder(order);
                var vendor = vendors.First();
                var customer = _customerService.GetCustomerById(order.CustomerId);
                var invoiceNumber = _documentNumberService.GetDocumentNumbers(
                    RunningNumberType.VendorShuqInvoice,
                    1,
                    null,
                    vendor.Id
                    );

                order.InvoiceNumber = invoiceNumber.First();

                _orderService.UpdateOrder(order);
                _workflowMessageService.SendOrderPlacedVendorNotification(order, vendor, _localizationSettings.DefaultAdminLanguageId);
                _workflowMessageService.SendOrderPlacedCustomerNotification(order, customer, _localizationSettings.DefaultAdminLanguageId);
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceOrderWithVendor(int vendorId, ProcessPaymentRequest processPaymentRequest, IEnumerable<int> itemIds, string attributeXml)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                //prepare order details
                var details = PreparePlaceOrderDetailsWithvendor(vendorId, processPaymentRequest, itemIds);
                details.CheckoutAttributesXml = attributeXml;
                //var processPaymentResult = GetProcessPaymentResult(processPaymentRequest, details);
                ...
                //if (processPaymentResult.Success)
                //{
                var order = SaveOrderDetailsWithVendor(processPaymentRequest, details);
                result.PlacedOrder = order;
        ...
        public virtual void PostPaymentProcessing(MasterOrder masterOrder)
        {
            if (masterOrder == null)
                throw new ArgumentNullException(nameof(masterOrder));

            var customer = _customerService.GetCustomerById(masterOrder.CustomerId);
            var carts = _shoppingCartService.GetShoppingCart(customer, ShoppingCartType.ShoppingCart, masterOrder.StoreId,onlySelectedItems:true);
            var orders = _orderService.GetOrdersByMasterOrderId(masterOrder.Id);
            foreach (var ord in orders)
            {
                var orderItems = _orderService.GetOrderItems(ord.Id);
                foreach (var ordItm in orderItems)
                {
                    var found = carts.Where(x => x.ProductId == ordItm.ProductId && x.AttributesXml == ordItm.AttributesXml).FirstOrDefault();
                    if (found != null)
                    {
                        // var product = _productService.GetProductById(found.ProductId);
                        _shoppingCartService.DeleteShoppingCartItem(found);
                        // _productService.AdjustInventory(product, -found.Quantity, found.AttributesXml, string.Format(_localizationService.GetResource("Admin.StockQuantityHistory.Messages.PlaceOrder"), ord.Id));

                    }
                }
            }

            _customerService.ResetCheckoutData(customer, masterOrder.StoreId, clearCouponCodes: true, clearCheckoutAttributes: true);
        }
        ...
        public virtual bool CanOrderPrepare(Order order)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
        ...
        public virtual bool CanOrderCancel(Order order)
        {
            IList<OrderItem> items = new List<OrderItem>();
            items = _orderService.GetOrderItems(order.Id);
            ...
            {
                ((List<ProductCategory>)productCategories).AddRange(_categoryService.GetProductCategoriesByProductId(item.ProductId));
            }

            //get category and parent category name
            var allCategories = _categoryService.GetAllCategories(showHidden: true);

            var currentMatchedCategories = allCategories
                .Where(x => productCategories.Any(y => y.CategoryId == x.Id))
                .ToList();

            categories.AddRange(currentMatchedCategories.Select(x => x.Name).ToList());

            while (currentMatchedCategories.Where(x => x.ParentCategoryId != 0).Any())
            {
                currentMatchedCategories = allCategories
                    .Where(x => currentMatchedCategories.Any(y => y.ParentCategoryId == x.Id))
                    .ToList();

                categories.AddRange(currentMatchedCategories.Select(x => x.Name).ToList());
            }

            bool isFrozen = categories.Contains("Frozen");
            bool isFresh = categories.Contains("Fresh");
            bool isStorablePerishable = categories.Contains("Storable Perishable");
            bool isNonPerishable = categories.Contains("Non-Perishable");

            DateTime expiryDate = order.CreatedOnUtc;
            DateTime ending = expiryDate.AddHours(23).AddMinutes(59).AddSeconds(59);
            var within24Hour = DateTime.Compare(ending, DateTime.UtcNow);
            ...
            if (within24Hour == 1 && isFresh && order.OrderStatus == OrderStatus.Processing)
                return true;

            if (within24Hour == 1 && isFrozen && order.OrderStatus == OrderStatus.Processing)
                return true;

            if (isFresh || isFrozen)
                return false;

            if (isStorablePerishable
        ...
        /// <summary>
        /// Gets a value indicating whether order is ready to set order received date.
        /// </summary>
        /// <param name="order"></param>
        /// <param name="shipments"></param>
        /// <returns></returns>
        public virtual bool IsReadyToReceive(List<Shipment> shipments, Order order)
        {
            var isShipped = shipments.Count > 0 && shipments.Any(x => x.DeliveryDateUtc == null) == false;
            var lastDeliveryDateUtc = shipments.Max(x => x.DeliveryDateUtc);
            ...
            var daysPassed = (DateTime.UtcNow - (DateTime)lastDeliveryDateUtc.Value).TotalDays;
            ...
            var returnRequest = _returnRequestService.GetGroupReturnRequestByOrderId(order.Id).FirstOrDefault();
        ...
        public virtual bool IsReturnRequestAllowed(Order order)
        {
            ...
            var shipment = _shipmentService.GetShipmentsByOrderId(order.Id).FirstOrDefault();
            var returnRequest = _returnRequestService.GetGroupReturnRequestByOrderId(order.Id).FirstOrDefault();

            if (returnRequest != null)
                return false;

            if (shipment == null || shipment.DeliveryDateUtc == null)
                return false;

            //product should be shipped
            if (order.ShippingStatusId != (int)ShippingStatus.Delivered)
                return false;
            ...
            var daysPassed = (DateTime.UtcNow - (DateTime)shipment.DeliveryDateUtc).TotalDays;
        ...
        public virtual bool CheckIfNeedReturn(Order order)
        {
            if (order == null || order.Deleted)
                return false;

            var groupReturnRequest = _returnRequestService.GetGroupReturnRequestByOrderId(order.Id);

            if (groupReturnRequest.Count == 0)
                return false;
            //status should be complete
            if (groupReturnRequest[0].ApproveStatusId == (int)ApproveStatusEnum.Approved)
                return true;
        ...
        /// <summary>
        /// set order received date and completed order
        /// </summary>
        /// <param name="shipments"></param>
        /// <param name="order"></param>
        /// <param name="notifyCustomer"></param>
        public virtual void ReceivedOrder(List<Shipment> shipments, Order order, bool notifyCustomer)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));

            if (!IsReadyToReceive(shipments, order))
                throw new NopException("Order not ready to recieved.");

            var vendor = _vendorService.GetVendorByOrderId(order.Id);
            order.ReceivedDateUtc = DateTime.UtcNow;
            _orderService.UpdateOrder(order);
            SetOrderStatus(order, OrderStatus.Complete, notifyCustomer);
            ...
            var orderCompletedAttachmentFileName = _orderSettings.AttachPdfInvoiceToOrderCompletedEmail ?
                "order.pdf" : null;
            _workflowMessageService.SendOrderCompletedCustomerNotification(order, order.CustomerLanguageId, orderCompletedAttachmentFilePath, orderCompletedAttachmentFileName);
            _workflowMessageService.SendOrderCompletedVendorNotification(order, vendor, order.CustomerLanguageId);
            _eventPublisher.Publish(new OrderCompletedEvent(order));
        }
