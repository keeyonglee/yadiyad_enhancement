...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        protected virtual decimal GetOrderSubtotalDiscountByVendor(Customer customer,
    decimal orderSubTotal, int vendorId, out List<Discount> appliedDiscounts)
        {
            appliedDiscounts = new List<Discount>();
            var discountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToOrderSubTotal, null, vendorId);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
            {
                foreach (var discount in allDiscounts)
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) &&
                        _discountService.ValidateDiscount(discount, customer).IsValid &&
                        (discount.MinimmumSubTotalAmount is null || orderSubTotal >= discount.MinimmumSubTotalAmount))
                    {
                        allowedDiscounts.Add(discount);
                    }
            }
        ...
        protected virtual decimal GetPlatformOrderSubtotalDiscount(Customer customer,
decimal orderSubTotalWithDiscount, decimal orderSubTotalWithoutDiscount, out List<Discount> appliedDiscounts)
        {
            appliedDiscounts = new List<Discount>();
            var discountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToPlatformOrderSubTotal);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
            {
                foreach (var discount in allDiscounts)
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) &&
                        _discountService.ValidateDiscount(discount, customer).IsValid &&
                        (discount.MinimmumSubTotalAmount is null || orderSubTotalWithoutDiscount >= discount.MinimmumSubTotalAmount))
                    {
                        allowedDiscounts.Add(discount);
                    }
            }

            appliedDiscounts = _discountService.GetPreferredDiscount(allowedDiscounts, orderSubTotalWithDiscount, out discountAmount);
        ...
        protected virtual decimal GetShippingDiscountByVendor(int vendorId, Customer customer, decimal shippingTotal, decimal subtotal, out List<Discount> appliedDiscounts)
        {
            appliedDiscounts = new List<Discount>();
            var shippingDiscountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToShipping, null, vendorId);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) &&
                        _discountService.ValidateDiscount(discount, customer).IsValid &&
                        (discount.MinimmumSubTotalAmount is null || subtotal >= discount.MinimmumSubTotalAmount))
                    {
                        allowedDiscounts.Add(discount);
                    }
        ...
        protected virtual decimal GetPlatformShippingDiscount(Customer customer, decimal shippingTotal, decimal subtotal, out List<Discount> appliedDiscounts)
        {
            appliedDiscounts = new List<Discount>();
            var shippingDiscountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToPlatformShipping);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) &&
        ...
        protected virtual decimal GetOrderTotalDiscountByVendor(Customer customer, decimal orderTotal, int vendorId, out List<Discount> appliedDiscounts)
        {
            appliedDiscounts = new List<Discount>();
            var discountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToOrderTotal, null, vendorId);
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) &&
        ...
        public virtual void GetShoppingCartSubTotalByVendor(IList<ShoppingCartItem> cart, bool includingTax, int vendorId,
            out decimal discountAmount, out List<Discount> appliedDiscounts,
            out decimal subTotalWithoutDiscount, out decimal subTotalWithDiscount)
        {
            GetShoppingCartSubTotalByVendor(cart, includingTax, vendorId,
                out discountAmount, out appliedDiscounts,
                out subTotalWithoutDiscount, out subTotalWithDiscount, out _);
        }
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            var discountAmountExclTax = (decimal)0;
            var appliedDiscount = new List<Discount>();
        ...
        public virtual void GetShoppingCartSubTotalByVendor(IList<ShoppingCartItem> cart,
    bool includingTax, int vendorId,
    out decimal discountAmount, out List<Discount> appliedDiscounts,
    out decimal subTotalWithoutDiscount, out decimal subTotalWithDiscount,
    out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            //We calculate discount amount on order subtotal excl tax (discount first)
            //calculate discount amount ('Applied to order subtotal' discount)
            var discountAmountExclTax = GetOrderSubtotalDiscountByVendor(customer, subTotalExclTaxWithoutDiscount, vendorId, out appliedDiscounts);
            if (subTotalExclTaxWithoutDiscount < discountAmountExclTax)
                discountAmountExclTax = subTotalExclTaxWithoutDiscount;
            var discountAmountInclTax = discountAmountExclTax;
            //subtotal with discount (excl tax)
        ...
        /// <returns>Shipping total</returns>
        public virtual decimal? GetShoppingCartShippingTotal(IList<ShoppingCartItem> cart, bool includingTax,
            out decimal taxRate, out List<Discount> appliedDiscounts, bool excludeDiscount = false)
        {
            ...
            else
            {
                ...
                var vendors = _vendorService.GetVendorsByProductIds(productIds);
                var products = _productService.GetProductsByIds(productIds);

                if (shippingAddress != null)
                {
                    var appliedDiscountsForAllVendors = new List<Discount>();
                    shippingTotal = vendors
                        .GroupBy(x => x.Id)
                        .Select(x => x.First())
                        .Select(vendor =>
                        {
                            var vendorAddress = _addressService.GetAddressById(vendor.AddressId);
                            var vendorProducts = products.Where(x => x.VendorId == vendor.Id).ToList();
                            var vendorState = "";
                            ...
                            var vendorCart = cart.Where(x => vendorProducts.Select(y => y.Id).Contains(x.ProductId)).ToList();

                            // subtotal per vendor
                            var subTotalIncludingTax = _workContext.TaxDisplayType == TaxDisplayType.IncludingTax && !_taxSettings.ForceTaxExclusionFromOrderSubtotal;
                            GetShoppingCartSubTotal(vendorCart, subTotalIncludingTax, out var orderSubTotalDiscountAmountBase, out var _, out var subTotalWithoutDiscountBase, out var _);

                            var shipmentCarrier = new ShipmentCarrierDTO
                            {
                                ProductName = string.Join(",", vendorProducts.Select(y => y.Name).ToList()),
                                SenderName = !string.IsNullOrEmpty(vendorAddress.FirstName) || !string.IsNullOrEmpty(vendorAddress.LastName) ? $"{vendorAddress.FirstName} {vendorAddress.LastName}" : vendor.Name,
                                ...
                                TotalWeight = vendorProducts.Select(y => y.Weight * cart.Where(z => z.ProductId == y.Id).First().Quantity).Sum(),
                                TotalValue = subTotalWithoutDiscountBase,
                                RequireInsurance = false, //
                                DeliveryModeId = (int)_orderService.ProductsHasDeliveryMode(products, DeliveryMode.Car),
                                Quantity = 1
                            };
                            ...
                            var shippingCarrierJsonStr = JsonConvert.SerializeObject(shipmentCarrier);
                            using var md5 = MD5.Create();
                            var hashArray = md5.ComputeHash(Encoding.UTF8.GetBytes(shippingCarrierJsonStr));
                            var hash = BitConverter.ToString(hashArray);
                            var hashStr = hash.Replace("-", string.Empty).ToLower();
                            var redisKey = _cacheKeyService.PrepareKey(NopShippingDefaults.ShippingQuotationCacheKey, hashStr);

                            var quotationStr = _staticCacheManager.Get(redisKey, () => "");

                            var shippingEstimatedCost = quotationStr == "" ? shippingCarrier.GetQuotation(shipmentCarrier) : JsonConvert.DeserializeObject<ShipmentCarrierEstimateCostDTO>(quotationStr);

                                if (quotationStr == "")
                                {
                                    _staticCacheManager.Set(redisKey, JsonConvert.SerializeObject(shippingEstimatedCost));
                                }
                                if (!shippingEstimatedCost.IsSuccess)
                                {
                                    if (shippingEstimatedCost.IsOverWeight)
                                        throw new ShipmentOverweightShippingException(shippingEstimatedCost.IsOverWeightMessage);
                                    else if (shippingEstimatedCost.CityNotAvailable)
                                        throw new NotInCoverageShippingException("This logistic service is not available in your city");
                                    else if (!shippingEstimatedCost.IsInsideCoverage)
                                        throw new NotInCoverageShippingException(vendor.Name + " does not cover the delivery address you have specified");
                                    else
                                        throw new ComputeShippingException("Failed to calculate shipping fee.");
                                }
                                else
                                {

                                    var shippingRate = shippingEstimatedCost.EstimatedPricePlusMarkUp;
                                    if (!excludeDiscount)
                                    {
                                        var appliedDiscountsOnVendor = new List<Discount>();
                                        shippingRate = AdjustShippingRate(shippingRate, vendorCart, out appliedDiscountsOnVendor);
                                        appliedDiscountsForAllVendors.AddRange(appliedDiscountsOnVendor);
                                    }
                                    return shippingRate;
                                }

                        }).Sum();
                    appliedDiscountsForAllVendors = appliedDiscountsForAllVendors.GroupBy(x => x.Id).Select(y => y.First()).ToList();
                    appliedDiscounts = appliedDiscountsForAllVendors;
                }
            }
        ...
        /// <summary>
        /// Gets shopping cart shipping total per vendor
        /// </summary>
        /// <param name="cart">Cart</param>
        /// <returns>Shipping total</returns>
        public virtual List<VendorShipping> GetShoppingCartShippingTotalPerVendor(IList<ShoppingCartItem> cart)
        {
            var productIds = cart.Select(x => x.ProductId).ToArray();
            var vendors = _vendorService.GetVendorsByProductIds(productIds);
            var products = _productService.GetProductsByIds(productIds);
            var deliveryByCar = false;
            //check whether any product got deliverymode as car
            foreach (var prod in products)
            {
                if (prod.DeliveryModeId == (int)DeliveryMode.Car)
                {
                    deliveryByCar = true;
                    break;
                }
            ...
            return vendors
                ...
                .Select(x =>
                {
                    if (x.CategoryId is null)
                        throw new NopException("Fail to identify vendor's category.");
                    var categoryVendor = _categoryService.GetCategoryById(x.CategoryId.Value);
                    decimal shipmentMaxWeight = _vendorService.GetVendorMaxShipmentWeight(categoryVendor.Name, deliveryByCar);

                    var vendorProducts = products.Where(y => y.VendorId == x.Id).ToList();
                    var vendorProductIds = vendorProducts.Select(y => y.Id).ToList();
                    var newCart = cart.Where(y => vendorProductIds.Contains(y.ProductId)).ToList();
                    decimal? shippingEstimatedCost = null;
                    var notInsideCoverage = false;
                    var failComputeShipping = false;
                    var notInsideCoverageCar = false;
                    var notInsideCoverageBike = false;
                    var shipmentOverweight = false;
                    try
                    {
                        shippingEstimatedCost = GetShoppingCartShippingTotal(newCart);
                    }
                    catch (NotInCoverageShippingException)
                    {
                        notInsideCoverage = true;
                        if (deliveryByCar)
                        {
                            notInsideCoverageCar = true;
                        }
                        else
                        {
                            notInsideCoverageBike = true;
                        }
                    }
                    catch (ComputeShippingException)
                    {
                        failComputeShipping = true;
                    }
                    catch (ShipmentOverweightShippingException)
                    {
                        shipmentOverweight = true;
                    }
                    return new VendorShipping
                    {
                        VendorId = x.Id,
                        ShippingCost = shippingEstimatedCost ?? 0,
                        NotInsideCoverage = notInsideCoverage,
                        FailComputeShipping = failComputeShipping,
                        NotInsideCoverageCar = notInsideCoverageCar,
                        NotInsideCoverageBike = notInsideCoverageBike,
                        ShipmentOverweight = shipmentOverweight,
                        MaxShipmentWeight = shipmentMaxWeight
                        //ErrorMessage = errorMessage,
                        //_orderTotalCalculationService.GetShoppingCartShippingTotalByVendor(cart)
                    };
                }).ToList();
        }

        public virtual List<VendorDiscount> GetShoppingCartDiscountTotalPerVendor(Customer customer)
        {
            var cart = _shoppingCartService.GetShoppingCart(customer, ShoppingCartType.ShoppingCart, _storeContext.CurrentStore.Id, onlySelectedItems: true);
            var productIds = cart.Select(x => x.ProductId).ToArray();
            var vendors = _vendorService.GetVendorsByProductIds(productIds);
            var products = _productService.GetProductsByIds(productIds);

            var vendorDiscounts = vendors
                .GroupBy(x => x.Id)
                .Select(x => x.First())
                .Select(x =>
                {
                    var appliedDiscounts = new List<Discount>();
                    var shippingDiscount = 0.0m;
                    var vendorProducts = products.Where(y => y.VendorId == x.Id).ToList();
                    var vendorProductIds = vendorProducts.Select(y => y.Id).ToList();
                    var newCart = cart.Where(y => vendorProductIds.Contains(y.ProductId)).ToList();
                    decimal? shippingEstimatedCost = null;

                    GetShoppingCartSubTotalByVendor(newCart, false, x.Id, out var subtotalDiscount, out var subtotalAppliedDiscounts, out var subTotalWithoutDiscountBase, out var subTotalWithDiscountBase);

                    try
                    {
                        shippingEstimatedCost = GetShoppingCartShippingTotal(newCart, _workContext.TaxDisplayType == TaxDisplayType.IncludingTax, out _, out appliedDiscounts, true);
                        shippingDiscount = GetShippingDiscountByVendor(x.Id, customer, (shippingEstimatedCost ?? 0), subTotalWithoutDiscountBase, out appliedDiscounts);
                    }
                    catch (NotInCoverageShippingException)
                    {
                    ...
                    var orderTotal = subTotalWithDiscountBase;
                    if (shippingEstimatedCost != null)
                    {
                        orderTotal += shippingEstimatedCost.Value;
                    }

                    //var discountAmount = GetOrderTotalDiscountByVendor(customer, orderTotal, x.Id, out var orderAppliedDiscounts);
                    var discountList = new List<Discount>();
                    discountList.AddRange(subtotalAppliedDiscounts);
                    //discountList.AddRange(orderAppliedDiscounts);
                    discountList.AddRange(appliedDiscounts);

                    return new VendorDiscount
                    {
                        VendorId = x.Id,
                        DiscountTotal = shippingDiscount + subtotalDiscount,
                        Discounts = discountList,
                        //OrderTotalDiscount = discountAmount,
                        SubTotalDiscount = subtotalDiscount,
                        ShippingDiscount = shippingDiscount,
                        PlatformShippingDiscount = 0m,
                        PlatformSubTotalDiscount = 0m,
                        SubTotalWithoutDiscountBase = subTotalWithoutDiscountBase,
                        ShippingEstimatedCost = shippingEstimatedCost != null ? shippingEstimatedCost.Value : 0m,
                    };
                }).ToList();

            // calcuate platform shipping discount
            var masterSubTotalWithoutDiscountBase = vendorDiscounts.Sum(vd => vd.SubTotalWithoutDiscountBase);
            var shippingTotal = vendorDiscounts.Where(vd => vd.OrderTotalDiscount == 0m && vd.SubTotalDiscount == 0m && vd.ShippingDiscount == 0m).Sum(vd => vd.ShippingEstimatedCost);
            if (shippingTotal > 0m)
            {
                var discountAmount = GetPlatformShippingDiscount(customer, shippingTotal, masterSubTotalWithoutDiscountBase, out var appliedPlatformShippingDiscounts);
                if (discountAmount > 0 && appliedPlatformShippingDiscounts.Count > 0)
                {
                    vendorDiscounts = vendorDiscounts.Select(vd =>
                        {
                            if (vd.OrderTotalDiscount == 0m && vd.SubTotalDiscount == 0m && vd.ShippingDiscount == 0m)
                            {
                                vd.PlatformShippingDiscount = _priceCalculationService.RoundPrice(vd.ShippingEstimatedCost / shippingTotal * discountAmount);
                                if (vd.PlatformShippingDiscount > vd.ShippingEstimatedCost)
                                {
                                    vd.PlatformShippingDiscount = vd.ShippingEstimatedCost;
                                }
                                vd.Discounts.AddRange(appliedPlatformShippingDiscounts);
                                vd.DiscountTotal += vd.PlatformShippingDiscount;
                            }
                            return vd;
                        })
                        .ToList();
            ...
            var masterSubTotalWithDiscountBase = vendorDiscounts.Where(vd => vd.OrderTotalDiscount == 0m && vd.SubTotalDiscount == 0m && vd.ShippingDiscount == 0m).Sum(vd => vd.SubTotalWithoutDiscountBase - vd.SubTotalDiscount);
            if (masterSubTotalWithDiscountBase > 0m)
            {
                var discountAmount = GetPlatformOrderSubtotalDiscount(customer, masterSubTotalWithDiscountBase, masterSubTotalWithoutDiscountBase, out var appliedPlatformSubTotalDiscounts);
                if (discountAmount > 0 && appliedPlatformSubTotalDiscounts.Count > 0)
                {
                    vendorDiscounts = vendorDiscounts.Select(vd =>
                    {
                        if (vd.OrderTotalDiscount == 0m && vd.SubTotalDiscount == 0m && vd.ShippingDiscount == 0m)
                        {
                            vd.PlatformSubTotalDiscount = _priceCalculationService.RoundPrice((vd.SubTotalWithoutDiscountBase - vd.SubTotalDiscount) / masterSubTotalWithDiscountBase * discountAmount);
                            if(vd.PlatformSubTotalDiscount > (vd.SubTotalWithoutDiscountBase - vd.SubTotalDiscount))
                            {
                                vd.PlatformSubTotalDiscount = vd.SubTotalWithoutDiscountBase - vd.SubTotalDiscount;
                            }
                            vd.Discounts.AddRange(appliedPlatformSubTotalDiscounts);
                            vd.DiscountTotal += vd.PlatformSubTotalDiscount;
                        }
                        return vd;
                    })
                        .ToList();
            ...
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            var discountPerVendor = GetShoppingCartDiscountTotalPerVendor(customer);
            var discounts = new List<Discount>();
            discountPerVendor.ForEach(x => discounts.AddRange(x.Discounts));
            appliedDiscounts = discounts.GroupBy(d => d.Id).Select(x => x.First()).ToList();
            discountAmount = discountPerVendor.Sum(x => x.DiscountTotal);
        ...
        public virtual decimal? GetShoppingCartTotalByVendor(int vendorId, IList<ShoppingCartItem> cart,
            out decimal discountAmount, out List<Discount> appliedDiscounts, out decimal shippingDiscount, out decimal platformShippingDiscount, out decimal platformSubtotalDiscount,
            out List<AppliedGiftCard> appliedGiftCards,
            out int redeemedRewardPoints, out decimal redeemedRewardPointsAmount,
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            //order total discount
            //discountAmount = GetOrderTotalDiscountByVendor(customer, resultTemp, vendorId, out appliedDiscounts);

            //order total discount
            var discountPerVendor = GetShoppingCartDiscountTotalPerVendor(customer);
            discountPerVendor = discountPerVendor.Where(x => x.VendorId == vendorId).ToList();
            var discounts = new List<Discount>();
            discountPerVendor.ForEach(x => discounts.AddRange(x.Discounts));
            appliedDiscounts = discounts;
            discountAmount = discountPerVendor.Sum(x => x.DiscountTotal);
            shippingDiscount = discountPerVendor.Select(x => x.ShippingDiscount).Sum();
            platformShippingDiscount = discountPerVendor.Select(x => x.PlatformShippingDiscount).Sum();
            platformSubtotalDiscount = discountPerVendor.Select(x => x.PlatformSubTotalDiscount).Sum();
