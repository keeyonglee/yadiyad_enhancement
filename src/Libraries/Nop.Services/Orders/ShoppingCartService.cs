...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        protected virtual void ClearCustomerShoppingCartCache(Customer customer)
        {
            var key = string.Format(NopOrderDefaults.CustomerShoppingCartCacheKey, customer.Id);
            _staticCacheManager.RemoveByPrefix(key);
        }
            ...
            int storeId, int quantity, bool addRequiredProducts, int shoppingCartItemId)
        {
            ...
            var cart = GetShoppingCart(customer, shoppingCartType, storeId, onlySelectedItems:true);
        ...
        /// <param name="onlySelectedItems">filter only the items selected for checkout, default returns all</param>
            ...
            bool onlySelectedItems = false)
        {
            ...
            var items = from sci in _sciRepository.Table
                        join p in _productRepository.Table on sci.ProductId equals p.Id
                        join v in _vendorRepository.Table on p.VendorId equals v.Id
                        orderby p.DisplayOrder, p.Id
                        where sci.CustomerId == customer.Id /*&& v.Online*/ && v.Active
                        select sci;
            ...
            // filter only items that are selected for checkout
            if(shoppingCartType == ShoppingCartType.ShoppingCart)
                if (onlySelectedItems)
                    items = items.Where(item => item.IsSelected);
            ...
            var key = _cacheKeyService.PrepareKeyForShortTermCache(NopOrderDefaults.ShoppingCartCacheKey, customer, shoppingCartType, storeId, productId, createdFromUtc, createdToUtc, onlySelectedItems);

            return _staticCacheManager.Get(key, () => items.ToList());
        }

        public virtual IList<ShoppingCartItem> GetShoppingCartWithItemIds(Customer customer, IEnumerable<int> itemIds, ShoppingCartType? shoppingCartType = null,
            int storeId = 0, int? productId = null, DateTime? createdFromUtc = null, DateTime? createdToUtc = null)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));

            var items = _sciRepository.Table.Where(sci => sci.CustomerId == customer.Id && itemIds.Contains(sci.Id));
            ...
            int quantity = 1, bool addRequiredProducts = true)
        {
            ...
            ClearCustomerShoppingCartCache(customer);
        ...
        public void UpdateCustomerSelectedSci(Customer customer, List<int> sciIds, ShoppingCartType? shoppingCartType = null,
            int storeId = 0, int? productId = null, DateTime? createdFromUtc = null, DateTime? createdToUtc = null, bool onlySelectedItems = false)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            if (customer == null)
            ...
            var items = _sciRepository.Table.Where(sci => sci.CustomerId == customer.Id).ToList();
            ...
            if (items.Count > 0)
            {
                var updatedItems = items
                    .Select(x =>
                    {
                        x.IsSelected = sciIds.Contains(x.Id);
                        return x;
                    })
                    .ToList();
                _sciRepository.Update(updatedItems);
            }
            // need to remove the shopping cart cache after changes
            ClearCustomerShoppingCartCache(customer);
        ...
        public virtual ShoppingCartItem GetShoppingCartItemById(int shoppingCartItemId)
        {
            if (shoppingCartItemId == 0)
                return null;

            return _sciRepository.GetById(shoppingCartItemId);
        }
        #endregion
    }
}