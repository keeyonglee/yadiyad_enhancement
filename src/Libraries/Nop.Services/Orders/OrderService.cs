...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual MasterOrder GetMasterOrderById(int masterOrderId)
        {
            if (masterOrderId == 0)
                return null;

            return _masterOrderRepository.Table.FirstOrDefault(o => o.Id == masterOrderId);
        }

        public virtual IList<Order> GetOrdersByMasterOrderId(int masterOrderId)
        {
            if (masterOrderId == 0)
                return new List<Order>();

            var query = from o in _orderRepository.Table
                        where o.MasterOrderId == masterOrderId && !o.Deleted
                        select o;
        ...
        public virtual IPagedList<Order> SearchOrders(int storeId = 0,
            ...
            bool isVendor = false, OrderSortBy? orderSortBy = null)
        {
            ...
            query = query.Where(o => !o.Deleted && o.PaymentStatusId != (int)PaymentStatus.Pending);
            if (isVendor)
            {
                query = query.Where(o => o.OrderStatusId != (int)OrderStatus.Pending);
            }

            if (orderSortBy == OrderSortBy.DeliverySlot)
            {
                query = query.OrderBy(o => o.OrderShippingDeliverySlot).ThenBy(o => o.CreatedOnUtc);
            }
            else
            {
        ...
        public virtual void UpdateMasterOrder(MasterOrder masterorder)
        {
            if (masterorder == null)
                throw new ArgumentNullException(nameof(masterorder));

            _masterOrderRepository.Update(masterorder);
        }

        public virtual MasterOrder CreateMasterOrderByOrders(List<Order> orders)
        {
            ...
            var masterOrder = new MasterOrder()
            {
                ...
                BillingAddressId = orders.First().BillingAddressId,
                PaymentStatusId = orders.First().PaymentStatusId,
                PaymentMethodSystemName = orders.First().PaymentMethodSystemName,
                CustomerCurrencyCode = orders.First().CustomerCurrencyCode,
                CurrencyRate = orders.First().CurrencyRate,
                OrderSubtotalInclTax = orders.Sum(x => x.OrderSubtotalInclTax),
                OrderSubtotalExclTax = orders.Sum(x => x.OrderSubtotalExclTax),
                OrderSubTotalDiscountInclTax = orders.Sum(x => x.OrderSubTotalDiscountInclTax),
                OrderSubTotalDiscountExclTax = orders.Sum(x => x.OrderSubTotalDiscountExclTax),
                OrderShippingInclTax = orders.Sum(x => x.OrderShippingInclTax),
                OrderShippingExclTax = orders.Sum(x => x.OrderShippingExclTax),
                PaymentMethodAdditionalFeeInclTax = orders.Sum(x => x.PaymentMethodAdditionalFeeInclTax),
                PaymentMethodAdditionalFeeExclTax = orders.Sum(x => x.PaymentMethodAdditionalFeeExclTax),
                TaxRates = string.Empty,
                OrderTax = orders.Sum(x => x.OrderTax),
                OrderDiscount = orders.Sum(x => x.OrderDiscount),
                OrderTotal = orders.Sum(x => x.OrderTotal),
                RefundedAmount = orders.Sum(x => x.RefundedAmount),
                CustomOrderNumber = string.Empty,
                TotalPlatformSubTotalDiscount = orders.Sum(x => x.PlatformSubTotalDiscount),
                TotalPlatformShippingDiscount = orders.Sum(x => x.PlatformShippingDiscount),
                TotalVendorShippingDiscount = orders.Sum(x => x.VendorShippingDiscount),
                //PaidDateUtc,
                //CustomValuesXml;
                //Deleted;
                CreatedOnUtc = DateTime.UtcNow,
            };
            _masterOrderRepository.Insert(masterOrder);

            masterOrder.CustomOrderNumber = _customNumberFormatter.GenerateMasterOrderCustomNumber(masterOrder);

            UpdateMasterOrder(masterOrder);
        ...
        public virtual bool HasPendingOrder(int customerId)
        {
            if (customerId == 0)
            ...
            var query = from o in _orderRepository.Table
                        ...
                        && o.CreatedOnUtc >= DateTime.UtcNow.AddMinutes(-8)
                        select o;

            return query.Count() > 0;
        }

        public List<Order> GetNumberOfOrdersWithoutShipment(int vendorId)
        {
            var query = from o in _orderRepository.Table
                        join oi in _orderItemRepository.Table on o.Id equals oi.OrderId
                        join p in _productRepository.Table on oi.ProductId equals p.Id
                        join s in _shipmentRepository.Table on o.Id equals s.OrderId into so
                        from s in so.DefaultIfEmpty()
                        where p.VendorId == vendorId && s == null && o.OrderStatusId == (int)OrderStatus.Processing
                        select o;
            return query.ToList();
        }

        public virtual DeliveryMode OrderHasDeliveryMode(int orderId, DeliveryMode deliveryMode = DeliveryMode.Bike)
        {
            if (orderId == 0)
            ...
            return query.Any(x => x.DeliveryMode == deliveryMode) ? deliveryMode : DeliveryMode.Bike;
        }

        public virtual DeliveryMode ProductsHasDeliveryMode(IList<Product> products, DeliveryMode deliveryMode = DeliveryMode.Bike)
        {
            if (!products.Any())
                throw new ArgumentNullException(nameof(products));

            return products.Any(x => x.DeliveryMode == deliveryMode) ? deliveryMode : DeliveryMode.Bike;
        }
        ...
        public virtual IList<OrderItem> GetOrderItemsByOrderVendorId(int orderId, int vendorId)
        {
        ...
        public virtual int GetTotalNumberOfReturnItemsInAllShipment(ReturnOrderItem returnOrderItem)
        {
            if (returnOrderItem == null)
                throw new ArgumentNullException(nameof(returnOrderItem));

            var totalInShipments = 0;
            var shipments = _shipmentService.GetShipmentsByReturnOrderId(returnOrderItem.ReturnOrderId);
        ...
        public virtual int GetTotalNumberOfReturnItemsCanBeAddedToShipment(ReturnOrderItem returnOrderItem)
        {
            if (returnOrderItem == null)
                throw new ArgumentNullException(nameof(returnOrderItem));

            var totalInShipments = GetTotalNumberOfReturnItemsInAllShipment(returnOrderItem);

            //TODO: add quantity in return order item
            var qtyOrdered = 1;
            var qtyCanBeAddedToShipmentTotal = qtyOrdered - totalInShipments;
            if (qtyCanBeAddedToShipmentTotal < 0)
                qtyCanBeAddedToShipmentTotal = 0;
