...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (!request.IsProOrganization)
            {
                //add to 'Individual' role
                var individualRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.IndividualRoleName);
                if (individualRole == null)
                    throw new NopException("'Individual' role could not be loaded");

                _customerService.AddCustomerRoleMapping(new CustomerCustomerRoleMapping { CustomerId = request.Customer.Id, CustomerRoleId = individualRole.Id });
            }
