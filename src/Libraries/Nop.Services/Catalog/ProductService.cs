...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        protected readonly IRepository<ProductReviewPictureMapping> _productReviewPictureMappingRepository;
        ...
        public ProductService(CatalogSettings catalogSettings,
            ...
            IRepository<ProductReviewPictureMapping> productReviewPictureMappingRepository,
            ...
            IVendorService vendorService)
        {
            ...
            _productReviewPictureMappingRepository = productReviewPictureMappingRepository;
        ...
        public virtual List<int> GetAllChildCategories(int id)
        {
            List<int> CategoryIds = new List<int>();
            CategoryIds.Add(id);

            var firstLayerCategory = _categoryService.GetAllCategoriesByParentCategoryId(id);
            if (firstLayerCategory.Count == 0)
                return CategoryIds;
            else
            {
                var secondLevelParentId = 0;
                foreach(var category in firstLayerCategory)
                {
                    CategoryIds.Add(category.Id);
                    var secondLayerCategory = _categoryService.GetAllCategoriesByParentCategoryId(category.Id);
                    if (secondLayerCategory.Count > 0)
                    {
                        foreach (var category2 in secondLayerCategory)
                            CategoryIds.Add(category2.Id);
                    }

                    secondLevelParentId = category.ParentCategoryId;
                }
            }
        ...
        public bool CheckIfIsEats(int categoryId)
        {
            List<int> eatCategoryIds = GetAllEatsCategory();

            if (eatCategoryIds.Contains(categoryId))
            {
                return true;
            }
            return false;
        }
        public bool CheckIfIsMart(int categoryId)
        {
            List<int> martCategoryIds = GetAllMartCategory();

            if (martCategoryIds.Contains(categoryId))
            {
                return true;
            }
            return false;
        }
        public List<int> GetAllEatsCategory()
        {
            var eatsId = _vendorSettings.ShuqEatsCategoryId;
            IList<Category> Category = _categoryService.GetAllCategories(0, false);
            List<int> CategoryIds = new List<int>();

            CategoryIds.Add(eatsId);

            foreach (var item in Category)
            {
                if (eatsId == item.ParentCategoryId)
                    CategoryIds.Add(item.Id);
                else
                {
                    var lastCategory = _categoryService.GetCategoryById(item.Id);
                    var nextCategory = _categoryService.GetCategoryById(item.ParentCategoryId);
                    if (nextCategory != null)
                    {
                        if (lastCategory.ParentCategoryId == nextCategory.Id)
                        {
                            if (nextCategory.ParentCategoryId == eatsId)
                            {
                                CategoryIds.Add(lastCategory.Id);
                            }
                        }
                    }
                }
        ...
        public List<int> GetAllMartCategory()
        {
            var martId = _vendorSettings.ShuqMartCategoryId;
            IList<Category> Category = _categoryService.GetAllCategories(0, false);
            List<int> CategoryIds = new List<int>();

            CategoryIds.Add(martId);

            foreach (var item in Category)
            {
                if (martId == item.ParentCategoryId)
                    CategoryIds.Add(item.Id);
                else
                {
                    var lastCategory = _categoryService.GetCategoryById(item.Id);
                    ...
                    if (nextCategory != null)
                    {
                        if (lastCategory.ParentCategoryId == nextCategory.Id)
                        {
                            if (nextCategory.ParentCategoryId == martId)
                            {
                                CategoryIds.Add(lastCategory.Id);
                            }
                        }
        ...
        public virtual IList<Product> GetInitialEatsProductsDisplayedOnHomepage()
        {
            List<int> cat = GetAllEatsCategory();
            ...
            var query = from p in _productRepository.Table
                        ...
                        v.Online &&
                        (v.OfflineUntil == null ||
                         v.OfflineUntil < DateTime.UtcNow)
                        select p;
        ...
        public virtual IPagedList<Product> GetAllEatsProductDisplayedOnHomePage(int pageIndex,
            int pageSize)
        {
            List<int> cat = GetAllEatsCategory();
        ...
        public virtual IList<Product> GetInitialMartProductsDisplayedOnHomepage()
        {
            List<int> cat = GetAllMartCategory();
        ...
        public virtual IPagedList<Product> GetAllMartProductDisplayedOnHomePage(int pageIndex,
            int pageSize)
        {
            List<int> cat = GetAllMartCategory();
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            ShuqBusinessNatureEnum category = ShuqBusinessNatureEnum.None,
            ...
            ProductSortingEnum orderBy = ProductSortingEnum.NameAsc,
            ...
            decimal? coverageDistance = null,
            ProductApprovalStatus? productApprovalStatus = null,
            bool? isVendorOnline = null)
        {
            return SearchProducts(out var _, false,
                pageIndex, pageSize, categoryIds, manufacturerId,
                storeId, vendorId, warehouseId,
                productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts, category,
                ...
                orderBy, showHidden, overridePublished, ratingMin, ratingMax, longitude, latitude, coverageDistance, productApprovalStatus, isVendorOnline);
        }
            ...
            )
        {
            ...
            if (category == ShuqBusinessNatureEnum.Eats)
            {
                List<int> eatCategoryIds = GetAllEatsCategory();
                categoryIds = eatCategoryIds;
                //eatCategoryIds.ForEach(x => categoryIds.Add(x));
            }
            else if (category == ShuqBusinessNatureEnum.Mart)
            {
                List<int> martCategoryIds = GetAllMartCategory();
                categoryIds = martCategoryIds;
                //martCategoryIds.ForEach(x => categoryIds.Add(x));
            }
            ...
            var pProductApprovalStatusId = SqlParameterHelper.GetInt32Parameter("ProductApprovalStatusId", (int?)productApprovalStatus);
            ...
            var pRatingMin = SqlParameterHelper.GetDecimalParameter("RatingMin", ratingMin);
            var pRatingMax = SqlParameterHelper.GetDecimalParameter("RatingMax", ratingMax);
            var pLongitude = SqlParameterHelper.GetDecimalParameter("Longitude", longitude);
            var pLatitude = SqlParameterHelper.GetDecimalParameter("Latitude", latitude);
            var pCoverageDistance = SqlParameterHelper.GetDecimalParameter("CoverageDistance", coverageDistance);
            ...
            var pVendorOnline = SqlParameterHelper.GetBooleanParameter("VendorOnline", isVendorOnline);
            ...
            var productsQuery = _productRepository.EntityFromSql("ProductLoadAllPaged",
                pCategoryIds,
                pManufacturerId,
                pStoreId,
                pVendorId,
        ...
        public virtual IList<Product> GetAdditionalProductsByVendorId(int productId, int vendorId = 0)
        {
            var query = _productRepository.Table;
            query = query.Where(x => x.VendorId == vendorId && x.Id != productId);
        ...
        public virtual IList<Product> GetProductsByVendorId(int vendorId)
        {
            var query = _productRepository.Table;
        ...
        public virtual void AdjustInventory(Product product, int quantityToChange, string attributesXml = "", string message = "")
        {
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStock)
            {
                ...
                if (quantityToChange < 0 && GetTotalStockQuantity(product) < product.NotifyAdminForQuantityBelow)
                {
                    ...
                    workflowMessageService.SendQuantityBelowVendorNotification(product, vendor, _localizationSettings.DefaultAdminLanguageId);
                    _eventPublisher.Publish(new LowStockProductEvent(product, vendor));
                }
            }
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                ...
                if (combination != null)
                {
                    ...
                    if (quantityToChange < 0 && combination.StockQuantity < combination.NotifyAdminForQuantityBelow)
                    {
                        ...
                        workflowMessageService.SendQuantityBelowVendorNotification(combination, vendor, _localizationSettings.DefaultAdminLanguageId);
                        _eventPublisher.Publish(new LowStockProductAttributeEvent(combination, vendor));
                    }
                }
            }
        ...
        public void InsertProductReviewPictureMapping(ProductReviewPictureMapping productReviewPictureMapping)
        {
            if (productReviewPictureMapping == null)
                throw new ArgumentNullException(nameof(productReviewPictureMapping));

            _productReviewPictureMappingRepository.Insert(productReviewPictureMapping);

            //event notification
            _eventPublisher.EntityInserted(productReviewPictureMapping);
        }

        public IList<int> GetProducReviewsPicturesById(int productReviewId)
        {
            if (productReviewId == 0)
                return new List<int>();

            var query = from prp in _productReviewPictureMappingRepository.Table
                        where prp.ProductReviewId == productReviewId
                        select prp.PictureId;
            return query.ToList();
        }

        public bool? CheckBuyerVotedHelpfulness(int productReviewId, int customerId)
        {
            if (productReviewId == 0)
                return null;
            ...
            var query = (from prh in _productReviewHelpfulnessRepository.Table
                        where prh.ProductReviewId == productReviewId && prh.CustomerId == customerId
                        select prh).FirstOrDefault();
            if (query != null)
            {
                return query.WasHelpful;
            }
            else
            {
                return null;
        ...
        public IPagedList<Product> GetProductsNotApproved(int pageIndex = 0, int pageSize = int.MaxValue, string keyword = null)
        {
            var query = from p in _productRepository.Table
                        where !p.Deleted && p.ProductApprovalStatusId == (int)ProductApprovalStatus.PendingApproval
                        select p;
