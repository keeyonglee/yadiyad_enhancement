...
namespace Nop.Plugin.StorageMedia.AmazonS3.Services
{
    ...
    /// Picture service
    /// </summary>
    public partial class AmazonS3PictureService : IPictureService
    {
        #region Fields

        private const string AWS_IMG_FILE_PATH = "pro/img/";
        ...
        public AmazonS3PictureService(INopDataProvider dataProvider,
        ...
        protected virtual byte[] LoadPictureFromFile(int pictureId, string mimeType)
        {
            ...
            var awsFilePath = GetAWSFilePath(picture);
            var binary = _amazonS3Service.GetObjectDataAsync(awsFilePath, false).Result;

            return binary;
        }

        public string GetAWSFilePath(Picture picture)
        {
            var lastPart = GetFileExtensionFromMimeType(picture.MimeType);
            var filePath = $"{AWS_IMG_FILE_PATH}{picture.SeoFilename}.{lastPart}";
        ...
        {
            var fileExtenstion = GetFileExtensionFromMimeType(mimeType);
            var picture = GetPictureById(pictureId);
            var fileName = $"{picture.SeoFilename}";

            _ = _amazonS3Service.UploadObjectAsync(
                new UploadFileRequest()
                {
                    FileName = fileName,
                    FolderPath = AWS_IMG_FILE_PATH,
                    FileBytes = pictureBinary,
        ...
        protected virtual void DeletePictureOnFileSystem(Picture picture)
        {
            ...
            var fileURL = GetAWSFilePath(picture);
            var deleteFileRequest = new DeleteFileRequest
            {
                FileUrl = fileURL
            };

            var response = _amazonS3Service.RemoveObjectAsync(deleteFileRequest, false).Result;

            if(response.Success == false)
            {
                _logger.Error($"Fail to delete file from AWS S3. File path: {fileURL}");
                //throw new Exception("Fail to delete file from AWS S3");
            }
        }
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            var lastPart = GetFileExtensionFromMimeType(picture.MimeType);
            var awsS3FileName = $"{picture.SeoFilename}.{lastPart}";
            var url = _amazonS3Service.GetPreSignedUrl(AWS_IMG_FILE_PATH+awsS3FileName, false);
            ...
            bool isNew = true, bool validateBinary = true)
        {
            ...
            picture.SeoFilename = GetUniqueSeoName(picture.Id, seoFilename);
        ...
        public virtual List<int> UploadPictureAndGetPictureId(IList<IFormFile> httpPostedFile, bool isPicture)
        {
            ...
            foreach (var file in httpPostedFile)
            {
                ...
                var picture = InsertPicture(file);
        ...
        public virtual Picture InsertPicture(IFormFile formFile, string defaultFileName = "", string virtualPath = "")
        {
            ...
            var picture = InsertPicture(_downloadService.GetDownloadBits(formFile), contentType, Path.GetFileNameWithoutExtension(fileName));
            var lastPart = GetFileExtensionFromMimeType(picture.MimeType);
            var awsS3FileName = $"{picture.SeoFilename}.{lastPart}";
            var result = _amazonS3Service.UploadObjectAsync(
                new UploadFileRequest()
                {
                    FileName = awsS3FileName,
                    FolderPath = AWS_IMG_FILE_PATH,
                    FormFile = formFile
                },
                false).Result;
            ...
            {
                throw new Exception(result.FileName);
            }
            ...
            bool isNew = true, bool validateBinary = true)
        {
            ...
            seoFilename = GetUniqueSeoName(picture.Id, currentName: seoFilename);
        ...
        private string GetUniqueSeoName(int pictureId, string currentName)
        {
            return $"{pictureId:000000}_{currentName}";
        }
        ...
        {
            if(mimeType.ToLower() == "image/svg+xml")
            {
                return pictureBinary;
            }
