...
namespace Nop.Plugin.Widgets.qBoSlider.Validators
{
    ...
    public class WidgetZoneModelValidator : BaseNopValidator<WidgetZoneModel>
    {
        public WidgetZoneModelValidator(ILocalizationService localizationService)
        {
            ...
            //RuleFor(x => x.MaxSlideWidgetZoneHeight).Must((model, maxHeight) =>
            //{
            //    return model.MaxSlideWidgetZoneHeight > model.MinSlideWidgetZoneHeight;
            //}).WithMessage(localizationService.GetResource("Nop.Plugin.Baroque.Widgets.qBoSlider.Admin.WidgetZone.MaxSlideWidgetZoneHeight.MustBeGreaterThanMinimumHeight"));
            //RuleFor(x => x.MaxSlideWidgetZoneHeight).Must((model, maxHeight) =>
            //{
            //    return model.MaxSlideWidgetZoneHeight - model.MinSlideWidgetZoneHeight >= 200;
            //}).WithMessage(string.Format(localizationService.GetResource("Nop.Plugin.Baroque.Widgets.qBoSlider.Admin.WidgetZone.MaxSlideWidgetZoneHeight.MustBeGreaterThanMinimumValueOnXPixels"), 200));
        }
    }
}
