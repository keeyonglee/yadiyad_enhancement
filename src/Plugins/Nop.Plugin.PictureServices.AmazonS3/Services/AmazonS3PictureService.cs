...
using YadiYad.Pro.Core.Domain.Amazon;
using YadiYad.Pro.Services.Services.Common;
using static SixLabors.ImageSharp.Configuration;


namespace Nop.Plugin.PictureServices.AmazonS3.Services
{
    /// <summary>
    /// Picture service
    /// </summary>
    public partial class AmazonS3PictureService : IPictureService
    {
        ...
        protected virtual void DeletePictureOnFileSystem(Picture picture)
        {
            ...
            var fileName = $"{picture.SeoFilename}.{lastPart}";
            var fileURL = AWS_IMG_FILE_PATH+ fileName;
            var deleteFileRequest = new DeleteFileRequest
            {
                FileUrl = fileName
            };

            var response = _amazonS3Service.RemoveObject(deleteFileRequest).Result;
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            var url = _amazonS3Service.GetPreSignedUrl(AWS_IMG_FILE_PATH+awsS3FileName);
        ...
        public virtual Picture InsertPicture(IFormFile formFile, string defaultFileName = "", string virtualPath = "")
        {
            ...
            var awsS3FileName = $"{picture.SeoFilename}";
;
            var result = _amazonS3Service.UploadObject(
