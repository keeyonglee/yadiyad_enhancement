...
var PageCancel = function PageCancel(opts) {
    var base = this;
    var url = opts.url;
    var model = {};
    var engagementParty;
    ...
    var returnPage;
    var $content = $('.content-job-cancel');
    var $form = $content.find('#form-job-cancel');

    base.getParentPage = function () {
        var page = null;
        if (typeof pageJobApplications !== "undefined") {
            page = pageJobApplications;
            ...
            returnPage = "/pro/jobapplication/list";
        }
        if (typeof pageJobApplicant !== "undefined") {
            ...
            returnPage = "/pro/jobapplication/hired";
        }
    ...
    //back-end interect
    base.submitJobCancel = function (callback) {

        var data = app.getFormValue($form);
        var engagementId = 0;
        if (engagementParty == "Buyer") {
            engagementId = parentPage.model.selectedJobEngagement.id;
        } else if (engagementParty == "Seller") {
            engagementId = parentPage.model.selectedJobApplication.id;
        }
        var settings = {
    ...
    base.handleResponse = function (response) {
        if (response && response.status && response.status.code === 1) {
            swal({
                icon: "success",
                title: "Job Request Cancelled Successfully"
            }).then(function () {
    ...
    base.setUserEvent = function () {
        $form.find("button[type=submit]").on("click", function (e) {
            ...
            if (base.validationForm()) {
                base.submitJobCancel(base.handleResponse);
            }
        });
    };
...
var pageCancel = new PageCancel({
    'url': {
        'setJobApplicationCancel': '/api/pro/jobapplication/{{id}}/cancel'
    }
});
