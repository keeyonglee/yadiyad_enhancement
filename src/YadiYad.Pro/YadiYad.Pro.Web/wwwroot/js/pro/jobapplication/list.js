var PageJobApplications = function (opts) {
    ...
    var model = opts.model || {
        ...
        setJobApplicationsResult: function (result) {
                    ...
                    if (item.appointmentStartDate && item.appointmentEndDate) {
                        ...
                        if (item.consultantAvailableTimeSlots) {
                            item.consultantAvailableTimeSlots.forEach(function (timeSlot, i) {
                                timeSlot.startDateText = moment(timeSlot.startDate, 'YYYY-MM-DDTHH:mm')
                                    .format('dddd');
                                timeSlot.startTimeText = moment(timeSlot.startDate, 'YYYY-MM-DDTHH:mm')
                                    .format('hh:mmA');
                                timeSlot.endTimeText = moment(timeSlot.endDate, 'YYYY-MM-DDTHH:mm')
                                    .format('hh:mmA');
        ...
        setSelectedJobApplication: function (jobApplication) {
    ...
    base.loadDataTable = function () {
        ...
        dtPayoutRequest = $content.find(".tbl-payout-request").DataTable({
            ...
            "ajax": {
                "url":
                    (model.selectedJobApplication.consultationProfile
                        ? url.getConsultationPayouRequests
                        : url.getPayouRequests)
                        .format({
                            id: model.selectedJobApplication.id
                        }),
                "type": "post",
                ...
                "dataFilter": function (data) {
                    ...
                    dataList.forEach(function (item) {
                        ...
                        item.action =
                            (item.endDate || item.jobMilestonePhase)
                            ?('<input class="btn btn-primary w-auto d-inline-block" value="' + buttonText + '" type="button"'
