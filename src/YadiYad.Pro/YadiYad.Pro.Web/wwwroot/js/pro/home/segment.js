var PageService = function (opts) {
    ...
    base.setCategoryExpertiseListResponse = function (target) {
        ...
        var otherObj = model.categoryExpertiseList.filter(obj => {
            return obj.name.toLowerCase() === 'others';
        });

        model.categoryExpertiseList = model.categoryExpertiseList.filter(obj => {
            return obj.name.toLowerCase() !== 'others';
        });
        ...
        model.categoryExpertiseList = model.categoryExpertiseList.map(function (item) {
            ...
            let data = item.expertises.reduce((r, e) => {
