...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual HomeCategoryModel PrepareHomeCategoryModel()
        {
            var categoryModel = new HomeCategoryModel();
            IList<CategoryModel> eatsCategory = new List<CategoryModel>();
            IList<CategoryModel> martCategory = new List<CategoryModel>();
            eatsCategory = PrepareHomepageEatsCategoryModels();
            martCategory = PrepareHomepageMartCategoryModels();

            foreach (var cat in eatsCategory)
            {
                categoryModel.EatsCategoryList.Add(cat);
            }

            foreach (var cat2 in martCategory)
            {
                categoryModel.MartCategoryList.Add(cat2);
            }
        ...
        public virtual CategoryModel PrepareCategoryModel(Category category, CatalogPagingFilteringModel command, List<int> categoryParams = null)
        {
            categoryParams = categoryParams ?? new List<int>();
            ...
            var model = new CategoryModel
            {
                ...
                IsEats = _productService.CheckIfIsEats(category.Id),
                IsMart = _productService.CheckIfIsMart(category.Id),
            };
            ...
            List<int> categoryIdss = new List<int>();

            //products
            IList<int> alreadyFilteredSpecOptionIds = model.PagingFilteringContext.SpecificationFilter.GetAlreadyFilteredSpecOptionIds(_webHelper);
            if (categoryParams.Count > 0)
            {
                foreach (var item in categoryParams)
                {
                    var childCategories = _productService.GetAllChildCategories(item);
                    categoryIdss.AddRange(childCategories);
                }
                    
            }
            else
                categoryIdss = _productService.GetAllChildCategories(category.Id);

            var products = _productService.SearchProducts(out var filterableSpecificationAttributeOptionIds,
                true,
                categoryIds: categoryIdss,
                storeId: _storeContext.CurrentStore.Id,
                visibleIndividuallyOnly: true,
        ...
        /// <summary>
        /// Prepare filtered category navigation model
        /// </summary>
        /// <param name="filteredCategoryId"></param>
        /// <param name="categoryLevel"></param>
        /// <returns></returns>
        public virtual CategoryNavigationModel PrepareFilteredCategoryNavigationModel(int filteredCategoryId, int startCategoryLevel)
        {
            //get active category
            var activeCategoryId = 0;
            ...
            var filteredCategories = GetFilteredSubCategory(cachedCategoriesModel, filteredCategoryId, startCategoryLevel);
            var model = new CategoryNavigationModel
            {
                CurrentCategoryId = activeCategoryId,
        ...
        /// <returns></returns>
        public List<CategorySimpleModel> GetFilteredSubCategory(
            List<CategorySimpleModel> categories,
            int filteredCategoryId,
            int categoryLevel,
            int currentLevel = 1)
        {
            var filteredCategories = new List<CategorySimpleModel>();

            foreach (var category in categories)
            {
                //only match one category
                if (category.Id == filteredCategoryId)
                {
                    filteredCategories = category.SubCategories;
                    var filteredSubCategories = filteredCategories;

                    currentLevel++;
                    while (currentLevel < categoryLevel)
                    {
                        currentLevel++;
                        filteredCategories = new List<CategorySimpleModel>();

                        foreach (var subCategory in filteredSubCategories)
                        {
                            filteredCategories.AddRange(subCategory.SubCategories);
                        }
                    }
                ...
                {
                    currentLevel++;
                    filteredCategories = GetFilteredSubCategory(
        ...
        public virtual IList<Category> GetAllEatsCategory()
        {
            IList<Category> secondLevelEatsCategory = new List<Category>();
            IList<Category> thirdLevelEatsCategory = new List<Category>();

            var eatsId = _vendorSettings.ShuqEatsCategoryId;

            secondLevelEatsCategory = _categoryService.GetAllCategoriesByParentCategoryId(eatsId);
            foreach (var item in secondLevelEatsCategory)
            {
                IList<Category> latestCategory = new List<Category>();
                latestCategory = _categoryService.GetAllCategoriesByParentCategoryId(item.Id);
                foreach (var item2 in latestCategory)
                {
                    thirdLevelEatsCategory.Add(item2);
                }
            }
            return thirdLevelEatsCategory;
        }

        public virtual IList<Category> GetAllMartCategory()
        {
            IList<Category> secondLevelMartCategory = new List<Category>();
            IList<Category> thirdLevelMartCategory = new List<Category>();

            var martId = _vendorSettings.ShuqMartCategoryId;

            secondLevelMartCategory = _categoryService.GetAllCategoriesByParentCategoryId(martId);
            foreach (var item in secondLevelMartCategory)
            {
                IList<Category> latestCategory = new List<Category>();
                latestCategory = _categoryService.GetAllCategoriesByParentCategoryId(item.Id);
                foreach (var item2 in latestCategory)
                {
                    thirdLevelMartCategory.Add(item2);
                }
            }
            return thirdLevelMartCategory;
        }
        ...
        public virtual List<CategoryModel> PrepareHomepageEatsCategoryModels()
        {
            var pictureSize = _mediaSettings.CategoryThumbPictureSize;
        ...
        public virtual List<CategoryModel> PrepareHomepageMartCategoryModels()
        {
            var pictureSize = _mediaSettings.CategoryThumbPictureSize;
        ...
        public virtual VendorModel PrepareVendorModel(Vendor vendor, CatalogPagingFilteringModel command)
        {
            ...
            var model = new VendorModel
            {
                ...
                JoinedDate = vendor.CreatedOnUtc,
            };

            var vendorThumbPictureSize = _mediaSettings.VendorThumbPictureSize;
            var pictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorPictureModelKey,
                vendor, vendorThumbPictureSize, true, _workContext.WorkingLanguage, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore);
            model.PictureModel = _staticCacheManager.Get(pictureCacheKey, () =>
            {
                ...
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                    ImageUrl = _pictureService.GetPictureUrl(ref picture, vendorThumbPictureSize),
            ...
            model.AboutUs = model.VendorAttributes.Where(x => x.Name == "About Us").Select(x => x.DefaultValue).FirstOrDefault();

            // breadcrumb
            var vendorBusinessNatureCategoryId = 0;
            var vendorBusinessNatureAttr = model.VendorAttributes.Where(x => x.Name == "Business Nature").Select(x => x.Values).FirstOrDefault();
            if (vendorBusinessNatureAttr != null)
            {
                var vendorBusinessNatureCategoryName = vendorBusinessNatureAttr.Where(x => x.IsPreSelected).Select(x => x.Name).FirstOrDefault();
                var category = _categoryService.GetCategoryByName(vendorBusinessNatureCategoryName);
                if (category != null)
                {
                    vendorBusinessNatureCategoryId = category.Id;
                }

                model.CategoryName = vendorBusinessNatureCategoryName.ToLower().Contains("eats") ? "Perishable" : "Non-Perishable";
            }
            model.Breadcrumb = PrepareVendortBreadcrumbModel(vendor, vendorBusinessNatureCategoryId);

            // picture
            var pictures = _vendorService.GetVendorPicturesByVendorId(vendor.Id);
            var vendorAboutUsPictureSize = _mediaSettings.VendorAboutUsPictureSize;
            var vendorSliderPictureSize = _mediaSettings.VendorSliderPictureSize;
            model.AboutUsPictureModel = pictures.Where(x => x.VendorPictureType == (int)VendorPictureType.Default).Select(x =>
            {
                var picture = _pictureService.GetPictureById(x.PictureId);
                return new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                    ImageUrl = _pictureService.GetPictureUrl(ref picture, vendorAboutUsPictureSize),
                    Title = string.Format(_localizationService.GetResource("Media.Vendor.ImageLinkTitleFormat"), picture.TitleAttribute),
                    AlternateText = string.Format(_localizationService.GetResource("Media.Vendor.ImageAlternateTextFormat"), picture.AltAttribute)
                };
            }).FirstOrDefault();
            model.SliderPictureModel = pictures.Where(x => x.VendorPictureType == (int)VendorPictureType.Slider).Select(x =>
            {
                var picture = _pictureService.GetPictureById(x.PictureId);
                return new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                    ImageUrl = _pictureService.GetPictureUrl(ref picture, vendorSliderPictureSize),
                    Title = string.Format(_localizationService.GetResource("Media.Vendor.ImageLinkTitleFormat"), picture.TitleAttribute),
                    AlternateText = string.Format(_localizationService.GetResource("Media.Vendor.ImageAlternateTextFormat"), picture.AltAttribute)
                };
            }).ToList(); ;

            // filter out no needed field
            model.VendorAttributes = null;
            ...
            // overview review
            model.VendorReviewOverview = PrepareVendorProductReviewOverviewModel(vendor);
            // product review
            model.VendorProductReviews = PrepareVendorAllProductReviewsModel(vendor);
        ...
        public virtual SearchModel PrepareSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            if (isSearchTermSpecified || true)
            {
                ...
                else
                {
                    ...
                    decimal? minPriceConverted = null;
                    decimal? maxPriceConverted = null;
                    decimal? minRatingConverted = null;
                    decimal? maxRatingConverted = null;
                    decimal? latitudeConverted = null;
                    decimal? LongitudeConverted = null;
                    ...
                    if (model.adv)
                    {
                        ...
                        var filterCategoryIds = model.cids;
                        if (filterCategoryIds != null
                            && filterCategoryIds.Count > 0)
                        {
                            categoryIds.AddRange(filterCategoryIds);
                        }
                        ...
                        //rating
                        if (!string.IsNullOrEmpty(model.rf) && !string.IsNullOrEmpty(model.rt))
                        {
                            if (decimal.TryParse(model.rf, out var minRating))
                                minRatingConverted = minRating;
                            if (decimal.TryParse(model.rt, out var maxRating))
                                maxRatingConverted = maxRating;
                        }

                        //rating
                        if (!string.IsNullOrEmpty(model.lat) && !string.IsNullOrEmpty(model.lng))
                        {
                            if (decimal.TryParse(model.lat, out var latitude))
                                latitudeConverted = latitude;
                            if (decimal.TryParse(model.lng, out var Longitude))
                    ...
                    products = _productService.SearchProducts(
                        out var filterableSpecificationAttributeOptionIds,
                        true,
                        ...
                        priceMin: minPriceConverted,
                        priceMax: maxPriceConverted,
                        keywords: string.IsNullOrWhiteSpace(searchTerms) ? null : searchTerms,
                        ...
                        vendorId: vendorId,
                        filteredSpecs: alreadyFilteredSpecOptionIds,
                        ratingMin: minRatingConverted,
                        ratingMax: maxRatingConverted,
                        latitude: latitudeConverted,
                        longitude: LongitudeConverted,
                        coverageDistance: model.CoverageDistance,
                        category: model.Category);



                    model.Products = _productModelFactory.PrepareProductOverviewModels(
                        products,
                        prepareSpecificationAttributes: true,
                        showState: model.ShowVendorState).ToList();
        ...
        /// <summary>
        /// Prepare the vendor breadcrumb model
        /// </summary>
        /// <param name="vendor">Vendor</param>
        /// <returns>Vendor breadcrumb model</returns>
        protected virtual VendorModel.VendorBreadcrumbModel PrepareVendortBreadcrumbModel(Vendor vendor, int businessNatureId)
        {
            if (vendor == null)
                throw new ArgumentNullException(nameof(vendor));

            var breadcrumbModel = new VendorModel.VendorBreadcrumbModel
            {
                Enabled = _catalogSettings.CategoryBreadcrumbEnabled,
                VendorId = vendor.Id,
                VendorName = _localizationService.GetLocalized(vendor, x => x.Name),
                VendorSeName = _urlRecordService.GetSeName(vendor)
            };

            var category = _categoryService.GetCategoryById(businessNatureId);
            if (category == null)
        ...
        /// <summary>
        /// Prepare vendor all product review overview model
        /// </summary>
        /// <param name="vendor">Vendor</param>
        /// <returns>Product review overview model</returns>
        protected virtual ProductReviewOverviewModel PrepareVendorProductReviewOverviewModel(Vendor vendor)
        {
            ProductReviewOverviewModel productReview;

            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorAllProductReviewsModelKey, vendor, _storeContext.CurrentStore);

            productReview = _staticCacheManager.Get(cacheKey, () =>
            {
                var productReviews = _productService.GetAllProductReviews(vendorId: vendor.Id, approved: true, storeId: _storeContext.CurrentStore.Id);
            ...
            {
                productReview.AllowCustomerReviews = false;
            }
        ...
        public virtual List<ProductReviewModel> PrepareVendorAllProductReviewsModel(Vendor vendor)
        {
            if (vendor == null)
                throw new ArgumentNullException(nameof(vendor));

            var list = new List<ProductReviewModel>();
