...
namespace Nop.Web.Factories
{
    ...
    public partial class CommonModelFactory : ICommonModelFactory
    {
        ...
        public virtual HeaderLinksModel PrepareShoppingCartModel()
        {
            var customer = _workContext.CurrentCustomer;
        ...
        public virtual AdminHeaderLinksModel PrepareAdminHeaderLinksModel()
        {
            ...
            var model = new AdminHeaderLinksModel
            {
                ...
                IsVendor = _customerService.IsVendor(customer)
            };
        ...
        public virtual ContactUsModel PrepareContactUsModel(ContactUsModel model, bool excludeProperties)
        {
            ...
            model.Subjects = Enum.GetValues(typeof(ContactUsSubject))
        ...
        public virtual string PrepareRobotsTextFile()
        {
            ...
            var robotsFilePath = _fileProvider.Combine(_fileProvider.MapPath("~/"),
                $"robots.custom.{_hostEnvironment.EnvironmentName.ToLower()}.txt");
            if (!_fileProvider.FileExists(robotsFilePath))
            {
                // use base custom robots file if not defined per environment
                robotsFilePath = _fileProvider.Combine(_fileProvider.MapPath("~/"), "robots.custom.txt");
            }
            ...
            else
            {
                ...
                // add shuq for url pattern to ignore
                var patternMatches = new string[] {string.Empty, "/shuq"};
                //localizable paths (without SEO code)
                foreach(var patternMatch in patternMatches)
                {
                    foreach (var path in localizableDisallowPaths)
                    {
                        sb.AppendFormat("Disallow: {0}{1}", patternMatch, path);
                        sb.Append(newLine);
                    }
                }
