...
namespace Nop.Web.Factories
{
    ...
    public partial class BlogModelFactory : IBlogModelFactory
    {
        ...
        public virtual void PrepareBlogPostModel(BlogPostModel model, BlogPost blogPost, bool prepareComments)
        {
            ...
            model.PictureUrl = blogPost.PictureId != null ? _pictureService.GetPictureUrl(blogPost.PictureId.Value) : "";
        ...
        public HomepageBlogPostModel PrepareHomepageBlogPostModel()
        {
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.HomepageBlogModelKey, _workContext.WorkingLanguage, _storeContext.CurrentStore);
            var cachedModel = _staticCacheManager.Get(cacheKey, () =>
            {
                var blog = _blogService.GetAllBlogPosts(_workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id);
                return new HomepageBlogPostModel
                {
                    WorkingLanguageId = _workContext.WorkingLanguage.Id,
                    BlogPost = blog
                        .Select(x =>
            ...
            var model = (HomepageBlogPostModel)cachedModel.Clone();
            foreach (var blogPostModel in model.BlogPost)
