...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class CommonModelFactory : ICommonModelFactory
    {
        ...
        public virtual YadiyadKeyStatisticsModel PrepareYadiyadKeyStatisticsModel()
        {
            var model = new YadiyadKeyStatisticsModel();
            model.NumberOfJobHiredThisMonth = _adminDashboardService.GetProJobHired(DateTime.UtcNow);
            model.NumberOfJobPostsThisMonth = _adminDashboardService.GetProJobPost(DateTime.UtcNow);
            model.NumberOfServiceHiredThisMonth = _adminDashboardService.GetProServiceHired(DateTime.UtcNow);
            model.NumberOfShuqOrdersThisMonth = _adminDashboardService.GetShuqOrders(DateTime.UtcNow);
            model.NumberOfJobHiredLastMonth = _adminDashboardService.GetProJobHired(DateTime.UtcNow.AddMonths(-1));
            model.NumberOfJobPostsLastMonth = _adminDashboardService.GetProJobPost(DateTime.UtcNow.AddMonths(-1));
            model.NumberOfServiceHiredLastMonth = _adminDashboardService.GetProServiceHired(DateTime.UtcNow.AddMonths(-1));
            model.NumberOfShuqOrdersLastMonth = _adminDashboardService.GetShuqOrders(DateTime.UtcNow.AddMonths(-1));

            return model;
        }

        public virtual VendorStatisticsModel PrepareVendorStatisticsModel()
        {
            var model = new VendorStatisticsModel();
            var vendor = _workContext.CurrentVendor;

            model.NumberOfOrdersLastMonth = _adminDashboardService.GetNumberOfOrdersByVendorId(vendor.Id, DateTime.UtcNow.AddMonths(-2), DateTime.UtcNow.AddMonths(-1));
            model.NumberOfOrdersLastWeek = _adminDashboardService.GetNumberOfOrdersByVendorId(vendor.Id, DateTime.UtcNow.AddDays(-14), DateTime.UtcNow.AddDays(-7));
            model.NumberOfOrdersThisMonth = _adminDashboardService.GetNumberOfOrdersByVendorId(vendor.Id, DateTime.UtcNow.AddMonths(-1), DateTime.UtcNow);
            model.NumberOfOrdersThisWeek = _adminDashboardService.GetNumberOfOrdersByVendorId(vendor.Id, DateTime.UtcNow.AddDays(-7), DateTime.UtcNow);
