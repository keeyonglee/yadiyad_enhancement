...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareOrderItemModels(IList<OrderItemModel> models, Order order)
        {
            ...
            foreach (var orderItem in orderItems)
            {
                ...
                var orderItemModel = new OrderItemModel
                {
                    ...
                    CategoryId = _categoryService.GetProductCategoriesByProductId(orderItem.ProductId)
                };
        ...
        public virtual OrderSearchModel PrepareOrderSearchModel(OrderSearchModel searchModel)
        {
            ...
            _baseAdminModelFactory.PrepareNumberOfDays(searchModel.AvailableDays);
        ...
        public virtual OrderListModel PrepareOrderListModel(OrderSearchModel searchModel)
        {
            ...
            var orders = _orderService.SearchOrders(storeId: searchModel.StoreId,
                ...
                isVendor: searchModel.IsLoggedInAsVendor);
            ...
            var model = new OrderListModel().PrepareToGrid(searchModel, orders, () =>
            {
                ...
                return orders.Select(order =>
                {
                    ...
                    if (searchModel.RequireCheckoutDeliveryDateAndTimeslot)
                    {
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            if (order != null)
            {
                ...
                model.CheckoutAttributesXml = order.CheckoutAttributesXml;
                model.CanPrepare = _orderProcessingService.CanOrderPrepare(order);
            ...
            bool excludeProperties = false)
        {
            if (shipment != null)
            {
                ...
                if (shipment.TotalWeight.HasValue)
                    model.TotalWeight =
                        $"{shipment.TotalWeight} [{_measureService.GetMeasureWeightById(_measureSettings.BaseWeightId)?.Name}]";
        ...
        public virtual VendorOrderShipmentModelList PrepareVendorOrderShipmentModel(OrderSearchModel searchModel)
        {
            var vendorId = _workContext.CurrentVendor.Id;
            var model = new List<VendorOrderShipmentModel>();

            var orders = _vendorDashboardService.OrdersPendingShipment(vendorId);

            foreach (var order in orders)
            {
                model.Add(new VendorOrderShipmentModel
                {
                    ...
                    CheckoutAttributeXML = order.CheckoutAttributeXML,
                    DueDate = order.DueDate,
                });
            }

            var pagedList = new PagedList<VendorOrderShipmentModel>(model, 0, searchModel.PageSize);

            //prepare list model
            var models = new VendorOrderShipmentModelList().PrepareToGrid(searchModel, pagedList, () => pagedList);
            return models;
        }

        public virtual VendorReturnRequestActionListModel PrepareVendorOrderReturnModel(OrderSearchModel searchModel)
        {
            var vendorId = _workContext.CurrentVendor.Id;
            var model = new List<VendorReturnRequestActionModel>();

            var orders = _vendorDashboardService.OrdersNeedReturnAction(vendorId);

            foreach (var order in orders)
            {
                model.Add(new VendorReturnRequestActionModel
                {
                    OrderId = order.OrderId,
                    GroupReturnRequestId = order.GroupReturnRequestId,
                    OrderDate = order.OrderDate,
                    ReturnRequestDate = order.ReturnRequestDate,
                    ActBefore = order.ActBefore
                });
            }

            var pagedList = new PagedList<VendorReturnRequestActionModel>(model, 0, searchModel.PageSize);

            //prepare list model
            var models = new VendorReturnRequestActionListModel().PrepareToGrid(searchModel, pagedList, () => pagedList);
            return models;
        }

        public virtual VendorProductSummaryListModel PrepareVendorTopSoldProductsModel(OrderSearchModel searchModel)
        {
            var vendorId = _workContext.CurrentVendor.Id;
            var model = new List<VendorProductSummaryModel>();

            var products = _vendorDashboardService.TopSoldProducts(vendorId);

            foreach (var product in products)
            {
                model.Add(new VendorProductSummaryModel
                {
                    ProductId = product.ProductId,
                    ProductName = product.ProductName,
                    Count = product.Count,
                    LastCreatedTime = product.LastCreatedTime
                });
            }

            var pagedList = new PagedList<VendorProductSummaryModel>(model, 0, int.MaxValue);

            //prepare list model
            var models = new VendorProductSummaryListModel().PrepareToGrid(searchModel, pagedList, () => pagedList);
            return models;
        }

        public virtual VendorProductSummaryListModel PrepareVendorTopReturnedProductsModel(OrderSearchModel searchModel)
        {
            var vendorId = _workContext.CurrentVendor.Id;
            var model = new List<VendorProductSummaryModel>();

            var products = _vendorDashboardService.TopReturnedProducts(vendorId);
