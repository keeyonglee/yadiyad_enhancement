...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        protected virtual List<SelectListItem> GetEatCategoryList(bool showHidden = true)
        {
            var categories = _categoryService.GetAllCategories(showHidden: showHidden);
            var lvl2Categories = categories.Where(x => x.ParentCategoryId == _vendorSettings.ShuqEatsCategoryId).Select(x => x.Id).ToList();
            var lvl3Categories = categories.Where(x => lvl2Categories.Contains(x.ParentCategoryId)).ToList();
            var listItems = lvl3Categories.Select(c => new SelectListItem
            {
                Text = _categoryService.GetFormattedBreadCrumbWithoutMain(c, categories),
                Value = c.Id.ToString()
            });
        ...
        protected virtual List<SelectListItem> GetMartCategoryList(bool showHidden = true)
        {
            var categories = _categoryService.GetAllCategories(showHidden: showHidden);
            var lvl2Categories = categories.Where(x => x.ParentCategoryId == _vendorSettings.ShuqMartCategoryId).Select(x => x.Id).ToList();
            var lvl3Categories = categories.Where(x => lvl2Categories.Contains(x.ParentCategoryId)).ToList();
            var listItems = lvl3Categories.Select(c => new SelectListItem
            {
                Text = _categoryService.GetFormattedBreadCrumbWithoutMain(c, categories),
        ...
        public virtual void PrepareCountries(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            //prepare available countries
            var availableCountries = _countryService.GetAllCountries(showHidden: false);
        ...
        public virtual void PrepareEatCategories(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available categories
            var availableCategoryItems = GetEatCategoryList();
            foreach (var categoryItem in availableCategoryItems)
            {
                items.Add(categoryItem);
            }
        ...
        public virtual void PrepareMartCategories(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available categories
            var availableCategoryItems = GetMartCategoryList();
            foreach (var categoryItem in availableCategoryItems)
            {
                items.Add(categoryItem);
            }
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareProductApprovalStatus(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available product types
            var availableProductApprovalStatusItems = ProductApprovalStatus.Draft.ToSelectList(false);
            foreach (var ProductApprovalStatusItem in availableProductApprovalStatusItems)
            {
                items.Add(ProductApprovalStatusItem);
            }
        ...
        public virtual void PrepareApproveStatus(IList<SelectListItem> items,
            bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            ...
            //prepare available return request statuses
            var availableStatusItems = ApproveStatusEnum.Pending.ToSelectList(false);
            foreach (var statusItem in availableStatusItems)
            {
                items.Add(statusItem);
            }
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareDeliveryModes(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available delivery dates
            var availableDeliveryModes = Enum.GetValues(typeof(DeliveryMode))
                   .Cast<DeliveryMode>()
            ...
            foreach (var date in availableDeliveryModes)
            {
                items.Add(new SelectListItem { Value = date.Value, Text = date.Text });
            };
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PreparePayoutBatchStatusList(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available request types
            var statusItems = PayoutBatchStatus.New.ToSelectList(false);
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PreparePayoutGroupStatusList(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available request types
            var statusItems = PayoutGroupStatus.New.ToSelectList(false);
            foreach (var statusItem in statusItems)
            {
                items.Add(statusItem);
            }
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PreparePayoutGroupCustomerList(int batchId, IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available customer
            var groups = _payoutBatchService.GetPayoutGroupsByBatchId(batchId);
            foreach (var group in groups)
            {
                var customerId = group.PayoutTo.ToString();
                var customerName = "";
                if (group.IndividualProfile != null)
                {
                    customerName = group.IndividualProfile.FullName;
                } else if (group.OrganizationProfile != null) {
                    customerName = group.OrganizationProfile.Name;
                } else if (group.Customer != null) {
                    customerName = group.Customer.Email;
                }
                items.Add(new SelectListItem { Value = customerId, Text = customerName });
            }
        ...
        public virtual void PrepareNumberOfDays(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available order statuses
            var availableDays = DaysEnum.OneDay.ToSelectList(false);
            foreach (var availableDay in availableDays)
            {
                items.Add(availableDay);
            }
