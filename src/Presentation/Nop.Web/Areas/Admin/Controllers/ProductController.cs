...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        private readonly ShippingSettings _shippingSetting;
        private readonly VendorSettings _vendorSettings;
        private readonly ProductSetupSettings _productSetupSettings;
        ...
        public ProductController(IAclService aclService,
            ...
            ProductSetupSettings productSetupSettings,
            LocalizationSettings localizationSettings)
        {
            ...
            _shippingSetting = shippingSetting;
            _productSetupSettings = productSetupSettings;
        ...
        protected virtual void UpdateProductPictureDisplayOrder(Product product, ProductPicture updatedPicture = null)
        {
            var displayOrder = updatedPicture?.DisplayOrder ?? 0;
            var updatedPictureId = updatedPicture?.PictureId ?? 0;
            var productPictures = _productService
                .GetProductPicturesByProductId(product.Id);
                
            var productPicturesAfterCurrent = productPictures.Where(s => s.DisplayOrder >= displayOrder
                                                                        && s.PictureId != updatedPictureId);
                
            foreach (var pp in productPicturesAfterCurrent.OrderBy(s => s.DisplayOrder))
            {
                pp.DisplayOrder = ++displayOrder;
            }

            displayOrder = 0;
            foreach (var pp in productPictures.OrderBy(s => s.DisplayOrder))
            {
                pp.DisplayOrder = ++displayOrder;
                _productService.UpdateProductPicture(pp);
            }
        }
        ...
        {
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToProducts, showHidden: true);
        ...
        protected virtual void EnforceProductSetting(Product product)
        {
            if (_productSetupSettings.AllowGroupedProductOnly == false)
            {
                product.VisibleIndividually = true;
            }

            if (_productSetupSettings.AllowChangeProductType == false)
            {
            ...
            if (_shippingSetting.IsShipMandatory)
            {
                product.IsShipEnabled = true;
            }
        }

        protected virtual ProductApprovalStatus? IdentifyProductUpdateStatus(Product product, ProductModel model)
        {
            ProductApprovalStatus? productApprovalStatus = null;

            if (model.ProductApprovalStatusId.HasValue
                && product.ProductApprovalStatusId != model.ProductApprovalStatusId
                )
            {
                productApprovalStatus = (ProductApprovalStatus)model.ProductApprovalStatusId;
            }
        ...
        public virtual IActionResult Create()
        {
            ...
            {
                var vendorModel = _vendorModelFactory.PrepareVendorModel(null, _workContext.CurrentVendor);
                    
                var businessNature = _vendorService.GetVendorAttributeValue(_workContext.CurrentVendor, NopVendorDefaults.VendorBusinessNature).Name;

                if (vendorModel.Address == null
                    || vendorModel.Address.Longitude == null
                    || vendorModel.Address.Latitude == null
                    || string.IsNullOrWhiteSpace(vendorModel.Address.Address1)
                    || string.IsNullOrWhiteSpace(vendorModel.Address.City)
                    || vendorModel.Address.StateProvinceId == null
                    || vendorModel.Address.CountryId == null
                    || (businessNature?.ToLower() != ShuqBusinessNatureEnum.Mart.GetDescription().ToLower()
                        && (vendorModel.VendorAttributes
                        .Where(x=>x.Name.Equals(NopVendorDefaults.VendorOperationDays, StringComparison.InvariantCultureIgnoreCase))
                        .FirstOrDefault()
                        ?.Values?
                    ...
                    || (businessNature?.ToLower() != ShuqBusinessNatureEnum.Mart.GetDescription().ToLower()
                            && (vendorModel.VendorAttributes
                        .Where(x => x.Name.Equals(NopVendorDefaults.VendorOperationHours, StringComparison.InvariantCultureIgnoreCase))
                        .FirstOrDefault()
                        ?.Values?
                        .Where(x => x.IsPreSelected == true)
                        .Count() ?? 0) <= 0))
                {
                    return View("RequiredVendorInfo");
                }
            }
            ...
            var model = _productModelFactory.PrepareProductModel(new ProductModel(), null, 
                allowedCategoryId: (_workContext.CurrentVendor?.CategoryId)??0);

            model.VendorCategoryId = _workContext.CurrentVendor?.CategoryId;
        ...
        public virtual IActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_workContext.CurrentVendor != null)
                {
                    model.AllowOrderSetting = _productModelFactory.CheckAllowOrderSetting(_workContext.CurrentVendor);
                }
                else if(model.VendorId != 0)
                {
                    var vendor = _vendorService.GetVendorById(model.VendorId);
                    model.AllowOrderSetting = _productModelFactory.CheckAllowOrderSetting(vendor);
                }

                if (model.AllowOrderSetting == false)
                {
                    model.AdvancedOrderDay = null;
                    model.MonMaxNoOrderQty = null;
                    model.TueMaxNoOrderQty = null;
                    model.WedMaxNoOrderQty = null;
                    model.ThuMaxNoOrderQty = null;
                    model.FriMaxNoOrderQty = null;
                    model.SatMaxNoOrderQty = null;
                    model.SunMaxNoOrderQty = null;
                }

                //product
                var product = model.ToEntity<Product>();

                if (_permissionService.Authorize(StandardPermissionProvider.ManageProductsAdvancedMode) == false
                    && _permissionService.Authorize(StandardPermissionProvider.ManageProductsOperatorMode) == false)
                {
                    product.Published = false;
                    product.AllowCustomerReviews = _productSetupSettings.DefaultAllowCustomerReviews;
                }

                if(product.Published)
                {
                    product.ProductApprovalStatus = ProductApprovalStatus.Approved;
                }
                else
                {
                    product.ProductApprovalStatus = ProductApprovalStatus.Draft;
                }

                product.CreatedOnUtc = DateTime.UtcNow;
                product.UpdatedOnUtc = DateTime.UtcNow;

                EnforceProductSetting(product);
                ...
                //warehouses
                if (_productSetupSettings.InventoryEnabled)
                {
                ...
                if(product.ProductApprovalStatus == ProductApprovalStatus.PendingApproval)
                {
                    //send email to operator on approval required
                    _workflowMessageService.SendProductPublishApprovalRequestOperatorNotification(
                        product,
            ...
            //prepare model
            model = _productModelFactory.PrepareProductModel(model, null, false,
                allowedCategoryId: (_workContext.CurrentVendor?.CategoryId) ?? 0);

            model.VendorCategoryId = _workContext.CurrentVendor?.CategoryId;

            model.IsShipEnabled = model.IsShipMandatory;
        ...
        public virtual IActionResult Edit(int id)
        {
            ...
            else
            {
                ...
                model.AllowOrderSetting = _productModelFactory.CheckAllowOrderSetting(vendor);
                model.VendorCategoryId = vendor.CategoryId;
            }
        ...
        [HttpPost, 
            ...
            ParameterBasedOnFormName("save-approve", "saveApprove"),
            ParameterBasedOnFormName("save-reject", "saveReject"),
            ParameterBasedOnFormName("save-request-publish", "saveRequestPublish"),
            ParameterBasedOnFormName("save-unpublish", "saveUnpublish")]
        public virtual IActionResult Edit(
            ProductModel model, 
            bool continueEditing, 
            bool saveApprove, 
            bool saveReject,
            bool saveRequestPublish,
            bool saveUnpublish)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            if (product.StockQuantity != model.LastStockQuantity && _productSetupSettings.InventoryEnabled)
            {
            ...
            model.IsShipMandatory = _shippingSetting.IsShipMandatory;


            #region product review

            var hasProductPublishApprovalRights = 
                _permissionService.Authorize(StandardPermissionProvider.ManageProductsAdvancedMode)
                || _permissionService.Authorize(StandardPermissionProvider.ManageProductsOperatorMode);

            if (hasProductPublishApprovalRights)
            {
                //action for operator and admin
                if (product.Published == false && saveApprove)
                {
                    model.Published = true;
                    model.ProductApprovalStatusId = (int)ProductApprovalStatus.Approved;
                }

                if (product.Published == false && saveReject)
                {
                    model.Published = false;
                    model.IsRejectAction = true;
                    model.ProductApprovalStatusId = (int)ProductApprovalStatus.Rejected;
                }
            }
            else
            {
                model.AllowCustomerReviews = _productSetupSettings.DefaultAllowCustomerReviews;
                model.Published = product.Published;
                //action for vendor
                if (product.Published == true && saveUnpublish)
                {
                    model.Published = false;
                    model.ProductApprovalStatusId = (int)ProductApprovalStatus.Unpublished;
                }

                if (product.Published == false && saveRequestPublish)
                {
                    model.Published = false;
                    model.ProductApprovalStatusId = (int)ProductApprovalStatus.PendingApproval;
                }
            }
            ...
            if(product.Published 
                && hasProductPublishApprovalRights == false)
            {
                isLimitedEdit = true;
            }
            ...
            if (ModelState.IsValid)
            {
                ProductApprovalStatus? productApprovalStatus = IdentifyProductUpdateStatus(product, model);

                if (_workContext.IsAdmin == true
                    && _workContext.CurrentVendor == null
                    && product.ProductApprovalStatus == ProductApprovalStatus.Approved
                    && product.Published
                    && model.Published)
                {
                    productApprovalStatus = ProductApprovalStatus.Approved;
                    model.ProductApprovalStatusId = (int)ProductApprovalStatus.Approved;
                    model.Published = true;
                }

                if (_permissionService.Authorize(StandardPermissionProvider.ManageProductsAdvancedMode)
                    ...
                    && model.Published)
                {
                    ...
                    && model.Published)
                {
                    model.ProductApprovalStatusId = product.ProductApprovalStatusId;
                    model.Published = product.Published;
                }
                ...
                //categories
                if (isLimitedEdit == false)
                {
                    int? vendorCategoryId = (_workContext.CurrentVendor?.CategoryId);

                    if (vendorCategoryId.HasValue)
                    {
                        if(vendorCategoryId == _vendorSettings.ShuqEatsCategoryId
                            || vendorCategoryId == _vendorSettings.ShuqMartCategoryId)
                        {
                            var allowedCategoryIds = _categoryService.GetChildCategoryIds(vendorCategoryId.Value);

                            var misMatchCategoryIds = model.SelectedCategoryIds
                                .Where(x => allowedCategoryIds.Any(y => y == x) == false)
                                .ToList();

                            if (misMatchCategoryIds.Count > 0)
                            {
                                throw new NopException(_localizationService.GetResource("Admin.Product.NotPermittedCategorySelected"));
                            }
                        }
                    }
                ...
                {
                    switch (productApprovalStatus.Value)
                    {
                        case ProductApprovalStatus.Approved:
                            //send email vendor on approval
                            _workflowMessageService.SendProductPublishApprovedStoreOwnerNotification(
                                product,
                                vendor,
                                _localizationSettings.DefaultAdminLanguageId);
                            break;
                        case ProductApprovalStatus.Rejected:
                            //send email vendor on reject
                            _workflowMessageService.SendProductPublishRejecteddStoreOwnerNotification(
                                product,
                                vendor,
                                _localizationSettings.DefaultAdminLanguageId);
                            break;
                        case ProductApprovalStatus.PendingApproval:
                            //send email operators on request publish
                            _workflowMessageService.SendProductPublishApprovalRequestOperatorNotification(
                                product,
                                _localizationSettings.DefaultAdminLanguageId);
                            break;
            ...
            string overrideAltAttribute, string overrideTitleAttribute, int productId)
        {
            ...
            if (productPictures.Any(p => p.PictureId == pictureId))
            ...
            _pictureService.SetSeoFilename(pictureId, _pictureService.GetPictureSeName(product.Name + $"_{displayOrder}"));
            ...
            _productService.InsertProductPicture(pp);
