    ...
    public class RoxyFilemanController : BaseAdminController
    {
        ...
        protected virtual async Task ProcessRequestAsync()
        {
            ...
            try
            {
                ...
                if(!IsActionAllowedForUser(action))
                    throw new Exception("You don't have required permission");
        ...
        protected virtual bool IsActionAllowedForUser(string action)
        {
            var restrictedActions = new List<string> { "COPYDIR", "COPYFILE", "DOWNLOAD", "DOWNLOADDIR", "MOVEDIR", "MOVEFILE", "RENAMEDIR", "RENAMEFILE" };
            if(restrictedActions.Contains(action))
                return false;
        
            var customer = _workContext.CurrentCustomer;
            var isAdmin = _customerService.IsInCustomerRole(customer, NopCustomerDefaults.AdministratorRoleName)
                || _customerService.IsInCustomerRole(customer, NopCustomerDefaults.OperatorRoleName);
            
            if(isAdmin)
                return true;
            
            var adminOnlyActions = new List<string>{ "DELETEDIR", "DELETEFILE" };
            return !adminOnlyActions.Contains(action);
        }
        #endregion
    }
}