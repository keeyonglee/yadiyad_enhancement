...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
    {
        ...
        [HttpPost, ActionName("GeneralCommon")]
        [FormValueRequired("changedataprotectionencryptionkey")]
        public virtual IActionResult ChangeDataProtectedEncryptionKey(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                if (model.SecuritySettings.DataProtectedEncryptionKey == null)
                    model.SecuritySettings.DataProtectedEncryptionKey = string.Empty;

                model.SecuritySettings.DataProtectedEncryptionKey = model.SecuritySettings.DataProtectedEncryptionKey.Trim();

                var newEncryptionPrivateKey = model.SecuritySettings.DataProtectedEncryptionKey;
                if (string.IsNullOrEmpty(newEncryptionPrivateKey) || newEncryptionPrivateKey.Length != 16)
                    throw new NopException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.DataProtectedEncryptionKey.TooShort"));

                var oldEncryptionPrivateKey = securitySettings.DataProtectedEncryptionKey;
                if (oldEncryptionPrivateKey == newEncryptionPrivateKey)
                    throw new NopException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.DataProtectedEncryptionKey.TheSame"));

                var protector = _dataProtector.CreateProtector("PersonalData");
                oldEncryptionPrivateKey = protector.Unprotect(oldEncryptionPrivateKey);

                securitySettings.DataProtectedEncryptionKey = protector.Protect(newEncryptionPrivateKey);
                _settingService.SaveSetting(securitySettings);

                _bankAccountService.UpdateEncryptionKey(oldEncryptionPrivateKey, oldEncryptionPrivateKey);

                _notificationService.SuccessNotification(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.DataProtectedEncryptionKey.Changed"));
            }
            catch (Exception exc)
            {
                _notificationService.ErrorNotification(exc);
