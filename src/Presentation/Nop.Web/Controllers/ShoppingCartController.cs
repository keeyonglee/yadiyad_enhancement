...
namespace Nop.Web.Controllers
{
    ...
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        [HttpsRequirement]
        public virtual IActionResult Cart(List<int> dfvid)
        {
            ...
            model = _shoppingCartModelFactory.PrepareShoppingCartModel(model, cart, getSupplierShippingDiscount:true);

            if(dfvid != null && dfvid.Count > 0)
            {
                foreach (var vendor in model.Vendors)
                {
                    vendor.DeliveryDateClash = dfvid.Any(x => x == vendor.Id);
                }
            }
            else
            {
                foreach (var vendor in model.Vendors)
                {
                    vendor.DeliveryDateClash = false;
                }
            }

            model.FromPage = "CartPage";
            return View(model);
        }
        ...
        public virtual IActionResult UpdateCart(IFormCollection form)
        {
            ...
            foreach (var warningItem in warnings.Where(warningItem => warningItem.Warnings.Any()))
            {
                ...
                var itemModel = model.Vendors.SelectMany(item => item.Items).FirstOrDefault(item => item.Id == warningItem.ItemId);
                if (itemModel != null)
        ...
        [HttpPost, ActionName("CartQty")]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult UpdateCartQty([FromBody]UpdateCartQtyModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart))
                return RedirectToRoute("Homepage");
            ...
            //get order items with changed quantity
            var itemsWithNewQuantity = cart.Where(x => x.Id == model.cartItemId).Select(item => new
            {
                NewQuantity = model.qty,
                Item = item,
                Product = products.ContainsKey(item.ProductId) ? products[item.ProductId] : null
            });
            ...
            scModel = _shoppingCartModelFactory.PrepareShoppingCartModel(scModel, cart);

            //update current warnings
            foreach (var warningItem in warnings.Where(warningItem => warningItem.Warnings.Any()))
            {
                //find shopping cart item model to display appropriate warnings
                var itemModel = scModel.Vendors.SelectMany(item => item.Items).FirstOrDefault(item => item.Id == warningItem.ItemId);
                if (itemModel != null)
                    itemModel.Warnings = warningItem.Warnings.Concat(itemModel.Warnings).Distinct().ToList();
            }
        ...
        public virtual IActionResult StartCheckout(IFormCollection form)
        {
            ...
            var products = _productService.GetProductsByIds(cart.Select(item => item.ProductId).Distinct().ToArray()).ToDictionary(item => item.Id, item => item);
            ...
            //try to update cart items with new quantities and get warnings
            var qtyWarnings = orderedCart.Select(cartItem => new
            {
                ItemId = cartItem.Item.Id,
                Warnings = _shoppingCartService.UpdateShoppingCartItem(_workContext.CurrentCustomer,
                    cartItem.Item.Id, cartItem.Item.AttributesXml, cartItem.Item.CustomerEnteredPrice,
            ...
            // set selected cartItems to session
            var selectedCartItemIds = form.ContainsKey("selectfromcart")
                ? form["selectfromcart"].ToString().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(int.Parse)
                .ToList()
                : new List<int>();
            _shoppingCartService.UpdateCustomerSelectedSci(_workContext.CurrentCustomer, selectedCartItemIds, 
                ShoppingCartType.ShoppingCart,
                _storeContext.CurrentStore.Id, 
                onlySelectedItems: true);

            //updated cart
            cart = _shoppingCartService.GetShoppingCart(_workContext.CurrentCustomer, ShoppingCartType.ShoppingCart, _storeContext.CurrentStore.Id, onlySelectedItems: true);

            //update current warnings
            var scModel = new ShoppingCartModel();
            var hasQtyWarning = false;
            scModel = _shoppingCartModelFactory.PrepareShoppingCartModel(scModel, cart);
            foreach (var warningItem in qtyWarnings.Where(warningItem => warningItem.Warnings.Any()))
            {
                //find shopping cart item model to display appropriate warnings
                hasQtyWarning = true;
                var itemModel = scModel.Vendors.SelectMany(item => item.Items).FirstOrDefault(item => item.Id == warningItem.ItemId);
            ...
            if (scModel.Vendors.Any(x=>x.DeliveryDateClash))
            {
                var vendorIds = scModel.Vendors.Where(x => x.DeliveryDateClash).Select(x=>x.Id).ToList();

                var strVendorIds = string.Join(',', vendorIds.Select(x => x.ToString()).ToList());

                return RedirectToRoute("ShoppingCart", new { dfvid = strVendorIds });
            }
        ...
        public virtual IActionResult UpdateWishlist(IFormCollection form)
        {
            ...
            var allIdsToRemove = form.ContainsKey("addtocart")
