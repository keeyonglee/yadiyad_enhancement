...
namespace Nop.Web.Controllers
{
    ...
    public partial class ProductController : BasePublicController
    {
        ...
        {
            bool productBelongToVendor = false;
            ...
            //if (vendor is null || !vendor.Active || !vendor.Online)
            //    invalidProduct = true;


            if (_workContext.CurrentVendor != null)
                if (_workContext.CurrentVendor.Id == product.VendorId)
                    productBelongToVendor = true;
            ...
            if (_workContext.CurrentVendor != null)
            {
                if ((product == null || product.Deleted || !product.Published || invalidProduct) && !productBelongToVendor)
                    return RedirectToRoute("Homepage");
            }
            else
            {
                if ((product == null || product.Deleted || !product.Published || invalidProduct) && !hasAdminAccess)
                    return RedirectToRoute("Homepage");
            }


            var reviewModel = new ProductReviewsModel();
            reviewModel = _productModelFactory.PrepareProductReviewsModel(reviewModel, product, null);
            ...
            model.ProductReviews = reviewModel;
            #endregion

            #region vendor review
            var vendorReviews = _productService.GetAllProductReviews(0, null, null, null, null, 0, 0, product.VendorId);
            model.VendorModel.RatingSum = vendorReviews.ToList().Sum(x => x.Rating);
            model.VendorModel.TotalReviews = vendorReviews.ToList().Count();
            #endregion

            model.ProductBusinessNature = model.Breadcrumb.CategoryBreadcrumb[0].Name.Contains("Eat") ? "Eat" : "Shuq";
            model.ProductReviewOverview.ProductBusinessNature = model.ProductBusinessNature;
            model.ProductPrice.ProductBusinessNature = model.ProductBusinessNature;
        ...
        [HttpsRequirement]
        public virtual IActionResult ShuqBuyerReview(int orderItemId)
        {
            ...
            var productToReview = _orderService.GetProductByOrderItemId(orderItemId);
            var picture = _productService.GetProductPicturesByProductId(productToReview.Id).FirstOrDefault();

            ViewData["ProductId"] = productToReview.Id;
            ViewData["OrderId"] = orderItem.OrderId;
            ViewData["ProductName"] = productToReview.Name;
            ViewData["ProductPictureUrl"] = picture != null ? _pictureService.GetPictureUrl(picture.PictureId) : "";
            ViewData["ProductQuantity"] = orderItem.Quantity;
            var totalAmount = orderItem.PriceInclTax;
            ViewData["ProductTotalAmount"] = Math.Round(totalAmount, 2);
            ViewData["MaxReviewImages"] = _mediaSettings.MaxReviewImages;

            return View();
        }

        public virtual IActionResult ProductReviews(int productId, int? pageNumber)
        {
            var product = _productService.GetProductById(productId);
            ...
            model = _productModelFactory.PrepareProductReviewsModel(model, product, pageNumber);
            //only registered users can leave reviews
            if (_customerService.IsGuest(_workContext.CurrentCustomer) && !_catalogSettings.AllowAnonymousUsersToReviewProduct)
                ModelState.AddModelError("", _localizationService.GetResource("Reviews.OnlyRegisteredUsersCanWriteReviews"));
            ...
            var productCategories = _categoryService.GetProductCategoriesByProductId(model.ProductId);
            ...
            model.ProductBusinessNature = breadcrumbModel.CategoryBreadcrumb[0].Name.Contains("Eat") ? "Eat" : "Mart";

            return View(model);
        }

        //public virtual IActionResult ShuqProductReviews(int productId)
        //{
        //    var product = _productService.GetProductById(productId);
        //    if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
        //        return RedirectToRoute("Homepage");
        ...
        [HttpPost("/api/shuq/products/productReviewShuqTest")]
        public IActionResult ProductReviewShuqTest()
        {
            return View();
        }

        [HttpPost("/api/products/shuqbuyerreviewadd")]
        [IgnoreAntiforgeryToken]
        public IActionResult ShuqBuyerReviewAdd([FromBody] ShuqBuyerReviewModel model)
        {
            ...
            foreach (var picId in model.ReviewPictureIds)
            {
                var entity = new ProductReviewPictureMapping
                {
                    ProductReviewId = productReview.Id,
                    PictureId = picId,
                    DisplayOrder = 1
                };
                _productService.InsertProductReviewPictureMapping(entity);
