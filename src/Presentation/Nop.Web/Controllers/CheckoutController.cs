...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual IActionResult Completed(int? orderId)
        {
            ...
            MasterOrder order = null;
            if (orderId.HasValue)
            {
                //load order by identifier (if provided)
                order = _orderService.GetMasterOrderById(orderId.Value);
            }
            //if (order == null)
            //{
            ...
            //model
            var model = _checkoutModelFactory.PrepareMasterCheckoutCompletedModel(order);
            return View(model);
        }
        ...
        public virtual IActionResult ShippingAddress()
        {
            ...
            if (cart.Where(x => x.IsSelected).Count() == 0)
            {
                return RedirectToRoute("ShoppingCart");
            }
        ...
        public virtual IActionResult SelectShippingAddress(int addressId, bool shipToSameAddress = false)
        {
            //validation
            if (_orderSettings.CheckoutDisabled)
                return RedirectToRoute("ShoppingCart");
            ...
            {
                _workContext.CurrentCustomer.BillingAddressId = _workContext.CurrentCustomer.ShippingAddressId;
                _customerService.UpdateCustomer(_workContext.CurrentCustomer);
                return RedirectToRoute("CheckoutConfirm");
            }
        ...
        public virtual IActionResult SelectBillingAddress(int addressId)
        {
            //validation
            if (_orderSettings.CheckoutDisabled)
                return RedirectToRoute("ShoppingCart");
        ...
        //submit delivery info & proceed to payment
        [IgnoreAntiforgeryToken]
        [HttpPost, ActionName("Confirm")]
        public virtual IActionResult ConfirmOrder(List<CustomCheckoutAttributeModel> customCheckoutAttr)
        {
            //validation
            if (_orderSettings.CheckoutDisabled)
                return RedirectToRoute("ShoppingCart");
            ...
            //create order according to selected cart item
            ...
            foreach (var vendor in cartModel.Vendors)
            {
                if (!vendor.Online || !vendor.Active)
                {
                    return Challenge();
                }
                ...
                var vendorAttrs = customCheckoutAttr.Where(x => x.VendorId == vendor.Id);
                if (vendorAttrs.Count() > 0)
                {
                    foreach (var vendorAttr in vendorAttrs)
                    {
                        ...
                        var vendorCheckoutAttribute = vendor.CheckoutAttributes.Where(x => x.Id == vendorAttr.AttributeId).FirstOrDefault();
                        if (
                            (
                                vendorCheckoutAttribute.AttributeControlType == AttributeControlType.Datepicker
                                && vendorCheckoutAttribute.MinDate > Convert.ToDateTime(vendorAttr.AttributeValue)
                                && vendorCheckoutAttribute.MaxDate < Convert.ToDateTime(vendorAttr.AttributeValue)
                                && vendorCheckoutAttribute.UnavailableDate.Any(x => Convert.ToDateTime(x) == Convert.ToDateTime(vendorAttr.AttributeValue))
                            )
                            ||
                            (
                                vendorCheckoutAttribute.AttributeControlType == AttributeControlType.DropdownList
                                && !vendorCheckoutAttribute.Values.Select(x => x.Id.ToString()).Contains(vendorAttr.AttributeValue)
                            )
                        )
                        {
                            throw new Exception("Received unexpected checkout attribute value.");
                        }
                ...
                var cartItemIds = vendor.Items.Where(x => x.IsSelected).Select(x => x.Id);
                if (cartItemIds.Count() > 0)
                {
                    var placeOrderResult = _orderProcessingService.PlaceOrderWithVendor(vendor.Id, processPaymentRequest, cartItemIds, attributesXml);
            ...
            var paymentURL = $"{paymentResponseURL}?r={redirectURL}&o={masterOrder.Id}";
        ...
        [HttpPost]
        public virtual IActionResult HasPendingOrder()
        {
            var hasPendingOrder = _orderService.HasPendingOrder(_workContext.CurrentCustomer.Id);
            return Json(new
            {
                havePendingOrder = hasPendingOrder
            });
        }
