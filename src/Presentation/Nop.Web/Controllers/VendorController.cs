...
namespace Nop.Web.Controllers
{
    public partial class VendorController : BasePublicController
    {
        ...
        [ValidateCaptcha]
        public virtual IActionResult ApplyVendorSubmit(ApplyVendorModel model, bool captchaValid, IFormCollection form)
        {
            if (!_vendorSettings.AllowCustomersToApplyForVendorAccount)
                return RedirectToRoute("Homepage");
            ...
            if (form.Files.Count > 0)
            {
                foreach(var file in form.Files)
                {
                    try
                    {
                        ...
                        var vendorPictureBinary = _downloadService.GetDownloadBits(file);
        ...
        public virtual IActionResult Info()
        {
            ...
            model = _vendorModelFactory.PrepareVendorInfoModel(model, null, false);
            return View(model);
        }
        ...
        public virtual IActionResult Info(VendorInfoModel model, IFormFile uploadedFile, IFormCollection form)
        {
            ...
            //If we got this far, something failed, redisplay form
            model = _vendorModelFactory.PrepareVendorInfoModel(model, null, true, vendorAttributesXml);
            return View(model);
        }
