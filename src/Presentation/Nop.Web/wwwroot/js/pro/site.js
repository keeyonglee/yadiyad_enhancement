...
$('body').on('hidden.bs.modal', '.modal', function (e) {
    ...
    if ($modalContent.hasClass('content-loaded')
        && !$(this).hasClass('retain')) {
...
$(document).ajaxError(function (event, jqxhr, settings, thrownError) {
    ...
    if (jqxhr
        && jqxhr.status === 401
        && settings.url.indexOf("token/exp") !== -1) {
...
app.initFormComponent = function ($form) {
    ...
    $form.find('[name]')
        ...
        .each(function () {
            ...
            if (type === 'radio') {
                ...
                if ($radioContainer.is(':last-child')
                ...
                if ($checkboxContainer.is(':last-child')
                    || $this.closest('.form-group ')
                        .find('.checkbox:last')
    ...
    $form.find('[type="date"], .form-control-datepicker')
    ...
    $form.find('[type="date"], .form-control-datepicker-start')
        .attr('autocomplete', 'off')
        .datepicker({
            autoclose: true,
    ...
    $form.find('[type="date"], .form-control-datepicker-end')
        .attr('autocomplete', 'off')
        .datepicker({
            autoclose: true
        }).on('changeDate', function (selected) {
    ...
    $form.find('[type="date"], .form-control-datepicker-tdp')
        .attr('autocomplete', 'off')
        .each(function () {
            if (this.datepicker) { return true; }
            const datepicker = new TheDatepicker.Datepicker(this);
            ...
            if (attrMaxDate) {
                ...
                else if (attrMaxDate.split('').reverse().join('').indexOf('y') === 0
                    && attrMaxDate.length > 1) {
    ...
    $form.find('.select')
        ...
        .each(function () {
            ...
            var enabledByDependent = function () {
            ...
            var disabledSelectedOption = function (options, excludingOptions) {
            ...
            var uniqueGroupFieldValues = function ($form, uniqueGroup, target) {
    ...
    $form.find('[type="hidden"].input-dropzone')
        .filter(':not(.' + classInitialed + '-dropzone)')
        .addClass(classInitialed + "-dropzone")
        .each(function () {
            var $this = $(this);
            ...
            $dropzone.dropzone({
                ...
                addedfile: function (file) {
                    ...
                    if (this.files.length) {
                        ...
                        for (i = 0, len = this.files.length; i < len; i++) {
                            ...
                            if (currentFile.name.toLowerCase() === file.name.toLowerCase()
                ...
                removedfile: function (file) {
                    ...
                    if (tempMultipleProperty && file.previewElement) {
                        ...
                        current = current.filter(function (e) { return e !== removed });
...
app.$setFieldValue = function (data, isWithInSubForm) {
    ...
    var type = $this.hasClass('input-dropzone') ? "dropzone" :
        (($this.hasClass('form-control-datepicker') ? 'date' : '')
            || ($this.hasClass('form-control-datepicker-tdp') ? 'date-tdp' : '')
            || ($this.hasClass('switch') ? 'switch' : '')
            || $this.attr('type')
            || ($this.is('select.select') ? 'select' : '')
            || ($this.is('textarea') ? 'textarea' : '')
            || ($this.is('select.tags') ? 'tags' : '')
            || ($this.is('label') ? 'label' : '')
            || ($this.is('span') ? 'span' : '')
            || ($this.is('table') ? 'table' : '')).toLowerCase();
    ...
    $form.find(fieldName ? '[name="' + fieldName + '"]' : '[name]').each(function () {
        ...
        var type = $this.hasClass('input-dropzone') ? "dropzone" :
            (($this.hasClass('form-control-datepicker') ? 'date' : '')
                ...
                || ($this.is('select.select') ? 'select' : '')
                || ($this.is('textarea') ? 'textarea' : '')
                || ($this.is('select.tags') ? 'tags' : '')).toLowerCase();
...
var dateComparer = {
    ...
    getDateDifferenceCompareToday: function (a) {
        var today = new Date();
