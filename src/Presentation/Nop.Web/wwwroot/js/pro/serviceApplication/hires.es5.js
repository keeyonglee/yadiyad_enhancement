...
var PageServiceHires = function PageServiceHires(opts) {
    var base = this;
    var url = opts.url;
    var msg = opts.msg;
            ...
            "ajax": {
                ...
                "dataFilter": function dataFilter(data) {
                    ...
                    dataList.forEach(function (item) {
                        ...
                        item.action = '<input class="btn btn-primary w-auto" value="' + buttonText + '" type="button"' + 'data-toggle="modal" data-target="#modal-payout-request"   ' + 'data-keyboard="false" data-backdrop="static"              ' + 'href="' + urlGetPayouRequest + '" />                      ';

                        if (item.status === 2) {
                            var urlGetInvoiceStatement = url.getInvoiceStatement.replace(/{{id}}/g, item.invoiceId);
                            item.action += '<br/><a class="text-primary" target="_blank"' + 'href="' + urlGetInvoiceStatement + '">[Invoice]</a>';
