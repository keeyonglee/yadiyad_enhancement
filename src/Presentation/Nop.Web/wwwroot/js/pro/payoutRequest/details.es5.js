...
var PageRequestService = function PageRequestService(opts) {
        ...
        base.handleResponse(response, function (response) {
            ...
            then(function () {
                ...
                if ($modal) {
                    ...
                    parentPage.reloadPayout && parentPage.reloadPayout() || parentPage.reloadSelectedEngagement();
                } else {
                    location.reload();
                }
            });
    ...
    base.onGetPayoutRequestResponse = function (response) {
        base.handleResponse(response, function (response) {
            ...
            if ((window.pageServiceHires || window.pageJobApplications) && (data.id === 0 || data.status === -2)) {
                $form.find('.btn-submit').removeClass('hidden');
            } else if ((window.pageServiceConfirms || window.pageJobApplicant) && data.id !== 0 && data.status === 0) {
            ...
            timeSheetScheduler = new WeeklyScheduler('#timesheet', timeSheetReadOnly, timeSheetViewMode);
            ...
            if (!data.isProrated) {
                ...
                $proratedWorkDuration.on('change', function () {
                    var maxMonthlyWorkDuration = 0.00;
                    var feePerDuration = (model.engagement.serviceProfileServiceFee || model.engagement.payAmount) + (model.engagement.serviceProfileOnsiteFee || 0);
                    if (model.engagement.serviceProfileServiceTypeId === 1 || model.engagement.jobType === 1) {
                        maxMonthlyWorkDuration = new Decimal(model.engagement.required || model.engagement.jobRequired).times(4).dividedBy(2);
                    } else if (model.engagement.serviceProfileServiceTypeId === 2 || model.engagement.jobType === 2) {
            ...
            if (data.id && data.statusName) {
                var classStatus = data.status === -3 ? "badge-gray" : data.status === -2 ? "badge-secondary" : data.status === -1 ? "badge-danger" : data.status === 0 ? "badge-gray" : data.status === 1 ? "badge-success" : data.status === 2 ? "badge-primary" : "";
