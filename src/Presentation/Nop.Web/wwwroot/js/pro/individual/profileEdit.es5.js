...
var PageIndividualProfileDetails = function PageIndividualProfileDetails(opts) {
    var base = this;
    var model = opts.model;
    var url = opts.url;
    var $content = $('.content-individual-profile');
    var $form = $content.find('#form-individual-profile-edit');
    ...
    base.setIndividualProfileData = function (response) {
        if (response && response.status && response.status.code === 1) {
            ...
            if (response.data.billingAddress.address1 != response.data.address1) addressMatch = false;else if (response.data.billingAddress.address2 != response.data.address2) addressMatch = false;else if (response.data.billingAddress.cityId != response.data.cityId) addressMatch = false;else if (response.data.billingAddress.countryId != response.data.countryId) addressMatch = false;else if (response.data.billingAddress.stateProvinceId != response.data.stateProvinceId) addressMatch = false;else if (response.data.billingAddress.zipPostalCode != response.data.zipPostalCode) addressMatch = false;
