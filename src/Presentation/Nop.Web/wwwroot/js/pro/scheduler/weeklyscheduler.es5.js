...
var WeeklyScheduler = function WeeklyScheduler(containerSelector, isReadOnly, viewMode, customDateRange, weekOptions) {
    var base = this;
    ...
    var isWithinAllowedDateRange = function isWithinAllowedDateRange(e) {
        ...
        if (minDate && maxDate) {
            ...
            if (minDate && minDate > e.start._date || maxDate && maxDate < e.start._date || minDate && minDate > e.end._date || maxDate && maxDate < e.end._date) {
                if (e.guide) {
    ...
    var notOverlapSchedule = function notOverlapSchedule(e) {
        var isScheduleOverlapped = false;

        $.each(cal._controller.schedules.items, function (i, item) {
            if (dateRangeOverlaps(e.start._date, e.end._date, item.start._date, item.end._date) && (!e.schedule || e.schedule && e.schedule.id !== item.id)) {
    ...
    base.setViewMode = function (mode) {
        ...
        $(containerSelector).removeClass('tui-layout-' + VIEW.WEEKLY).removeClass('tui-layout-' + VIEW.WEEK).removeClass('tui-layout-' + VIEW.MONTHLY);
