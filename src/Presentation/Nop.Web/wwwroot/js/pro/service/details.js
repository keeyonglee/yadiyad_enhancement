var PageServiceProfileDetails = function (opts) {
    var base = this;
    ...
    var $templateUserAgreement =
        ...
        '</div>'+
        '</div>';
    var $templateremoteServiceModel =
        '<div class="col-12">' +
        ...
        '<div class="radio d-inline-block mr-4">' +
        '<label class="col-form-label form-check-label">' +
        '<input type="radio" name="serviceModelId" value="3" required="" class="form-init" data-val="true">' +
        '  Remote' +
        '</label>' +
        '</div>' +
        '</div>' +
    ...
    var $templateLicenseCert =
        '<div class="section-inner-licenseCert card card-body">' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Name</label>' +
        '<div class="col-sm-9">' +
        '<input type="text" class="form-control" name="professionalAssociationName{{id}}" maxlength="200" required />' +
        '</div>' +
        '</div>' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Issuing Organization</label>' +
        '<div class="col-sm-9">' +
        '<input type="text" class="form-control" name="licenseCertificateName{{id}}" maxlength="200" required />' +
        '</div>' +
        '</div>' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Document</label>' +
        '<div class="col-sm-9">' +
        '<input name="licenseCertificateDocument{{id}}" class="form-control input-dropzone" type="hidden" data-name-origin="downloadId" data-name-temp="newDownloadId" required />' +
        '</div>' +
        '</div>' +
        '<div class="row">' +
        '<ul class="col-sm-6 text-muted" style="list-style-type: circle; padding-left:20px;"><i>' +
        '<li>If both fields are not applicable, click ‘Remove’</li>' +
        '<li>If one field is not applicable, enter ‘NA’</li> ' +
        '</i></ul>' +
        '<div class="col-sm-6 text-right">' +
        '<button type="button" class="btn btn-default btn-remove-licenseCert" onclick="this.offsetParent.offsetParent.remove();"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;&nbsp; Remove</button>' +
        '</div>' +
        '</div>' +
        '</div>';

    var $templateEducation =
        '<div class="section-inner-education card card-body">' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Qualification</label>' +
        '<div class="col-sm-9">' +
        '<select class="form-control select2 select" required' +
        ' name="academicQualificationTypeId{{id}}" textName="academicQualificationTypeName{{id}}"' +
        ...
        ' name="academicQualificationName{{id}}" maxlength="200" required />' +
        '</div>' +
        '</div>' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Institution (Optional)</label>' +
        '<div class="col-sm-9">' +
        '<input type="text" class="form-control" maxlength="200"' +
        ' name="academicInstitution{{id}}" />' +
        '</div>' +
        '</div>' +
        '<div class="form-group">' +
        '<div class="float-right">' +
        '<button type="button" class="btn btn-default btn-remove-education" onclick="this.offsetParent.remove();"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;&nbsp; Remove</button>' +
        '</div>' +
        '</div>' +
        '</div>';

    var $templateLanguage =
        '<div class="section-inner-language card card-body">' +
        '<div class="form-group row">' +
        '<label for="" class="col-sm-3 col-form-label">Language</label>' +
        '<div class="col-sm-9">' +
        '<select class="form-control select2 select" required' +
        ' name="languageId{{id}}" textName="languageName{{id}}"' +
        ...
        '<div class="col-sm-9">' +
        '<select class="form-control select2 select" required' +
        ' name="proficiencyLevel{{id}}" textName="proficiencyLevelName{{id}}"' +
        ...
        '<div class="col-sm-9">' +
        '<select class="form-control select2 select" required' +
        ' name="proficiencyWrittenLevel{{id}}" textName="proficiencyWrittenLevelName{{id}}"' +
        ' placeholder="Proficiency"' +
        ' data-sourceurl="/api/pro/source/LanguageProficiency"' +
        ' data-id="id" data-text="name">' +
        '</select>' +
        ...
        '<ul class="col-sm-6 text-muted text-left" style="list-style-type: circle; padding-left:20px;"><i>' +
        '<li>If not applicable, click ‘Remove’</li>' +
        '</i></ul>' +
        '<div >' +
        '<button type="button" class="btn btn-default btn-remove-language" onclick="this.offsetParent.remove();"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;&nbsp; Remove</button>' +
        '</div>' +
        '</div>' +
        '</div>';
    ...
    base.submitServiceProfile = function (callback) {
        ...
        var settings = {
            "url": model.id
                ? url.updateServiceProfile.format(model)
                : url.createServiceProfile.format(model),
            "method": 'POST',
            "headers": {
                "Content-Type": "application/json"
            },
        ...
        $form.find('[name="serviceTypeId"]').on('change', function (e, isInit) {
            ...
            switch (value) {
                case "1":
                    ...
                    if (isInit)
                        if (serviceModelIdVal != 0)
                            $content.find('[name="serviceModelId"]').val([serviceModelIdVal]);
                    break;
                case "2":
