...
(function ($) {
    ...
    function createRatingEl($input, options) {
        ...
        if (options.inline) {
            ...
            $ratingEl.addClass(options.additionalClasses);
        } else {
        ...
        if (options.clearable && !options.readonly) {
            $ratingEl.append('&nbsp;').append('<a class="' + clearClass + '">' + '<i class="' + options.iconLib + ' ' + options.clearableIcon + '" >' + '</a>');
        }
        return $ratingEl;
    }

    var Rating = function Rating(input, options) {
    ...
    Rating.prototype = {
        ...
        highlight: function highlight(value, skipClearable) {
