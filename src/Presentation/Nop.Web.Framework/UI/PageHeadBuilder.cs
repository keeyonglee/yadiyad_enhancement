    ...
    public partial class PageHeadBuilder : IPageHeadBuilder
    {
        ...
        public static string Version
        {
            get
            {
                if (string.IsNullOrWhiteSpace(_version))
                {
                    ...
                    _version = lastModified.ToString("yyMMddHHmmss");
                }
        ...
        public virtual void AppendScriptParts(ResourceLocation location, string src, string debugSrc, bool excludeFromBundle, bool isAsync)
        {
            ...
            _scriptParts[location].Insert(0, new ScriptReferenceMeta
            {
                ...
                IsAsync = isAsync,
                Src = AppendVersion(src),
                DebugSrc = AppendVersion(debugSrc)
            });
        }
        /// <summary>
        ...
        public virtual string AppendVersion(string src)
        {
            if(string.IsNullOrWhiteSpace(src) == false)
            {
                src = src + (src.Contains("?") ? "&" : "?") + $"v={Version}";
            }
